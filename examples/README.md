...
| [`indexer-from-zero`](./indexer-from-zero) | [Streams](https://secondlayer.tools/streams) | An indexer from the **raw** inputs — signed parquet dumps for cold history, then a checkpointed live tail. Plus a `secondlayer streams pull` + DuckDB one-liner. |
