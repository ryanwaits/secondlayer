...
| `secondlayer-integrity-alert.{service,timer}` | live `/health/integrity` `gaps_unfillable` \| `chain_unlinked` → Slack `--force` (one page per incident, `--recovery` on healthy); `gaps_detected` / `degraded` do not page; curl/JSON failure stays quiet | every 5 min |
...
The five health units answer different questions and none subsumes another:
...
instead of months, `integrity-alert` is the live `/health/integrity` page for
`gaps_unfillable` \| `chain_unlinked` (distinct from daily `canonical-audit`).
`gaps_detected` / `degraded` do not page. `--force` skips Jev; no gateway key
...
sudo systemctl enable --now \
  ...
  secondlayer-integrity-alert.timer \
...
journalctl -u secondlayer-integrity-alert.service -n 50 --no-pager
tail -n 50 /var/log/secondlayer-staging-health.log
tail -n 50 /var/log/secondlayer-integrity-alert.log
...
sudo systemctl start secondlayer-integrity-alert.service
