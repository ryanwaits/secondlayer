...
// Tuple keys become camelCase on `event.data`. Declaring `prints` on a source
...
export default defineSubgraph({
	...
	sources: {
		...
		addLiq: {
			...
			contractId: POOL,
			topic: "add-liquidity",
			prints: { "add-liquidity": { amountA: "uint", amountB: "uint" } },
		},
	...
	handlers: {
		swap(event, ctx) {
			// `prints` types event.data: tokenIn/tokenOut → string, amounts → bigint.
			ctx.insert("swaps", {
		...
		addLiq(event, ctx) {
			ctx.insert("liquidity_adds", {
				...
				amountA: event.data.amountA,
				amountB: event.data.amountB,
			});
		},
