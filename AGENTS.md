...
- `/extended` if present is a read-only view of Index rows on :3999, off
  by default. Never mount it under `/v1`. Never proxy node `/v2` through
  it. Never restore a Hiro PG dump or run `stacks-blockchain-api`. Stay
  current by tracking their OpenAPI, not by pinning their image.
