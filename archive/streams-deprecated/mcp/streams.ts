	...
	defineTool<{ name: string; endpointUrl: string; filters: Filter[] }>(
		...
		"Create a new webhook stream with filters.",
		{
			...
			filters: z
				.array(FilterSchema)
				.min(1)
				.describe("Event filters (at least one required)"),
		},
				...
				content: [
					{
						type: "text",
						text: JSON.stringify(
							{ id: result.stream.id, signingSecret: result.signingSecret },
							null,
							2,
						),
					},
				...
				content: [
					{
						type: "text",
						text: JSON.stringify(
							{ id: stream.id, status: stream.status },
							null,
							2,
						),
					},
	...
	defineTool<{ confirm?: boolean }>(
		...
		"Pause all active streams. Without confirm: true, returns a preview of streams that would be paused.",
		{
			confirm: z
				.boolean()
				.optional()
				.describe("Set to true to execute. Omit or false for preview only."),
		},
		async ({ confirm }) => {
			if (!confirm) {
				const { streams } = await getClient().streams.list({
					...
					content: [
						{
							type: "text",
							text: JSON.stringify(
								{ preview: true, count: names.length, streams: names },
								null,
								2,
							),
						},
