...
function mapQueryRow(r: any): {
	...
	const metrics: StreamMetrics | null =
		r.total_deliveries != null
			? {
					stream_id: r.id,
...
app.patch("/:id", async (c) => {
	...
	if (parsed.endpointUrl !== undefined)
		updates.endpoint_url = parsed.endpointUrl;
	if (parsed.filters !== undefined)
		updates.filters = jsonb(parsed.filters) as any;
	if (parsed.options !== undefined) {
		updates.options = jsonb({
			...parseJsonb<object>(existing.options),
			...parsed.options,
		}) as any;
	}
...
app.post("/:id/enable", async (c) => {
	...
	if (existing.status !== "inactive" && existing.status !== "failed") {
		throw new ValidationError(
			`Cannot enable stream with status '${existing.status}'`,
		);
	}
...
app.post("/:id/pause", async (c) => {
	...
	if (existing.status !== "active") {
		throw new ValidationError(
			`Cannot pause stream with status '${existing.status}'`,
		);
	}
...
app.post("/:id/resume", async (c) => {
	...
	if (existing.status !== "paused") {
		throw new ValidationError(
			`Cannot resume stream with status '${existing.status}'`,
		);
	}
...
app.post("/:id/replay-failed", async (c) => {
	...
	const totalFailed =
		failedDeliveryBlocks.length === MAX_FAILED_REPLAY
			? ((
					await db
						.selectFrom("deliveries")
