...
| Field | Value |
|---|---|
| Phase | **1: Runes decoder spike** (done; Gate 1 pass). Phase 2 next: ≥14 days at tip, reorg handled, oss boots from empty |
...
| Last updated | 2026-09-26 (plan 056 step 5: C3 at 968,557 confirmed 0/0 against `ord`; Gate 1 PASS; ord released to follow tip) |
...
| 1 | Decode diff 0 mismatches on the sample; entry + balance diff 0 at C1, C2, C3 (D17); supply invariant holds at every flush 840,000 → C3; backfill wall time + PG size recorded; fail-closed tests pass (invariant break, prev-hash break) | **PASS (2026-09-26):** decode diff 0 mismatches (~101,282 txs sampled: block 840,000 full, 840,001-840,021 full, block 850,000 full, one-per-10k 850,000-960,000, 840,022-840,099 not separately covered); C1 (841,000) 0 entry / 0 balance mismatches; C2 (900,000) 0 entry / 0 balance mismatches, on the replayed DB (plan 051), adopted as main 2026-09-25 (plan 056 step 0); C3 (968,557) 0 entry / 0 balance mismatches, rune counts 215,215 = 215,215, outpoint counts 6,941,422 = 6,941,422 (plan 056 step 4). Backfill wall time + PG size recorded at all three checkpoints |
...
| D12 | 2026-09-22 | Tip following via ZMQ (`hashblock` + `rawblock`) on node-server bitcoind, no polling. Needs a prod bitcoind config change + restart (brief burn-feed gap for prod stacks-node; schedule a window). ZMQ is unauthenticated: publish only on the private allowlist, same DOCKER-USER pattern as `:8332`, never `0.0.0.0/0`. Lands as its own step in the Phase 1 plan. **Amended 2026-09-26:** tip following wakes on bitcoind's `waitfornewblock` RPC (blocking, authenticated, no extra port), with a `getbestblockhash` fallback on nodes that lack it. ZMQ dropped: `zeromq` crashes Bun (oven-sh/bun#18546), and RPC-only works for self-hosters without ZMQ. 038 no longer changes ZMQ | LOCKED |
...
| Backfill 841,000→900,000, complete (plan 039 step 7) | First run (now DB `btc_pre_replay`) finished in 10,221.7s, peak rss about 13.7 GB, 0 integrity errors. Two live-caught defects fixed mid-run: (1) `runes` container `mem_limit` 6g to 16g after a cgroup OOM-kill at FLUSH_INTERVAL=1000 in the still-dense post-launch window (host had 29GB free, not a host-wide OOM, Stacks IBD unaffected); (2) `rune_entries.symbol` mapped to `null` for a literal U+0000 codepoint (Postgres text columns reject a raw NUL byte, a Runestone Symbol tag legally allows codepoint 0), crashed the flush with Postgres error 22021, fixed with a regression test, does not affect the 841,000 digest/state-hash proof (no such etching existed in 840k-841k). That run was later found missing 9 etchings (see State parity C2 row); the replay into `runes_c2` from the 841,000 snapshot took 21,402.0s and is the adopted state. Final stats on the adopted DB (`btc` as of 2026-09-25): PG size 31 GB; rune_entries 178,862; rune_balances 7,793,613; rune_events 180,848,037; rune_block_digests 59,001; digest at 900,000 `ef51dea8d4084c5278a22618607729062b496a21e911f6051e51036bb729e3f7` | 2026-09-25 | `docker logs runes-backfill-900k`, `docker logs runes-replay-c2` on `stacks-feeder`; `pg_database_size`, row counts, `rune_block_digests` on DB `btc` |
...
| State parity C3 = 968,557 (plan 056) | H = bitcoind tip (968,563) minus 6, ord frozen at H via `--height-limit 968558` before our backfill passed it. Backfill 900,000 to 968,557 in two legs: leg 1 at FLUSH_INTERVAL=1000 reached checkpoint 935,000 in 13,205s then was OOM-killed by its own cgroup (`runes` container `mem_limit` is 22g, not the 16g recorded in the 900k row above; per-flush rss stayed ~10-16GB, the kill came from the in-window buffer between flushes on dense blocks, peak anon-rss 21.9GB at kill, reviewer diagnosis); leg 2 (resumed from checkpoint 935,000) cut FLUSH_INTERVAL to 250, completed to 968,557 in 16,980.4s with 0 errors, peak rss 16,719MB across the leg. Total backfill time 900k to H: 30,185.4s (~8h23m) of active backfill work (excludes the OOM investigation/restart gap). No new OOM beyond the one leg-1 kill (dmesg count 3 to 4, that single event); `/data/feeder` stayed 4-5% throughout; Stacks feeder tenure_height rose the whole time (96,263 to 101,706 across the run, no stall), never restarted. Final stats on `btc`: PG size 70 GB; rune_entries 215,215; rune_balances 7,223,989; rune_events 439,451,013; rune_block_digests 127,558. Ord dump at H: `.err` files empty, JSON valid, ord active and unchanged at H 30s after restart. `parity-state` at 968,557: runes 215,215 = 215,215, outpoints 6,941,422 = 6,941,422, entry mismatches 0, balance mismatches 0. Ord released (height-limit removed), confirmed rising past H to 968,598. Phase 3 sizing fact: FLUSH_INTERVAL=1000 is not safe at this block density even with a 22g `mem_limit`; FLUSH_INTERVAL=250 is | 2026-09-26 | `@secondlayer/bitcoin` `backfill --to 968557` and `parity-state` on `stacks-feeder`; `docker logs runes-backfill-c3`; `/data/feeder/btc-parity/968557-{ord-runes,ord-balances}.json`, `968557-diff.json` |
