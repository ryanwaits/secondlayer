...
| block-metadata-correlated-subqueries | LOW | new | Per-row scalar subqueries for block_time/burn_block_height | `LEFT JOIN blocks b ON b.height=t.block_height AND b.canonical` selecting once. |
| trait-filtering-absent-at-index | LOW | shipped-partial | No `trait=` param on Index event/call reads | Wire `resolveTraitContractIds` into `INDEX_EVENT_CONFIG`/contract-calls (registry + `/v1/contracts?trait=` already ship). |
