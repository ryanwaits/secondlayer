...
export function validateDocs(
	...
	for (const [name, funcDoc] of doc.functions) {
		...
		const funcDiagnostics = validateFunctionDoc(
			funcDoc,
			abi?.functions.find((f: AbiFunction) => f.name === name),
		);
...
function validateFunctionDoc(
	...
	if (abiFunc) {
		const abiArgNames = new Set(
			abiFunc.args.map((a: { name: string }) => a.name),
		);
...
export function calculateCoverage(
	...
	const publicFunctions = abi.functions.filter(
		(f: AbiFunction) => f.access !== "private",
	);
	const totalFunctions = publicFunctions.length;
	const documentedFunctions = publicFunctions.filter((f: AbiFunction) =>
		doc.functions.has(f.name),
	).length;

	const totalMaps = abi.maps?.length || 0;
	const documentedMaps = (abi.maps || []).filter((m: { name: string }) =>
		doc.maps.has(m.name),
	).length;

	const totalVariables = abi.variables?.length || 0;
	const documentedVariables = (abi.variables || []).filter(
		(v: { name: string; access: string }) => {
	...
	return {
		...
		documentedVariables,
		variableCoverage:
			totalVariables > 0 ? (documentedVariables / totalVariables) * 100 : 100,
