...
export function generateMarkdown(
	...
	// Documentation link
	if (doc.header.uri) {
		const hashNote = doc.header.docsHash
			? ` (hash: \`${doc.header.docsHash}\`)`
			: "";
		lines.push(
			`**Documentation:** [${doc.header.uri}](${doc.header.uri})${hashNote}`,
		);
		lines.push("");
	}

	// Separate error constants from regular constants
	const errorConstants = Array.from(doc.constants.values()).filter(
		(c) => c.isError,
	);
	const regularConstants = Array.from(doc.constants.values()).filter(
		(c) => !c.isError,
	);
	...
	if (includeToc) {
		...
		if (errorConstants.length > 0)
			lines.push("- [Error Constants](#error-constants)");
		...
		for (const func of functions) {
			lines.push(
				...generateFunctionMarkdown(
					func,
					abi?.functions.find(
						(f: AbiContract["functions"][number]) =>
	...
	if (doc.maps.size > 0) {
		...
		for (const map of doc.maps.values()) {
			lines.push(
				...generateMapMarkdown(
					map,
					abi?.maps?.find((m: { name: string }) => m.name === map.mapName),
					includeTypes,
				),
			);
			lines.push("");
	...
	if (errorConstants.length > 0) {
		...
		for (const errConst of errorConstants) {
			...
			lines.push(
				`| ${code} | \`${errConst.variableName}\` | ${errConst.errorDescription || errConst.desc || "-"} |`,
			);
		}
		lines.push("");
	}
		...
		for (const variable of regularConstants) {
			lines.push(
				...generateVariableMarkdown(
					variable,
					abi?.variables?.find(
						(v: { name: string }) => v.name === variable.variableName,
					),
					includeTypes,
				),
			);
...
function generateFunctionMarkdown(
	...
	if (func.params.length > 0) {
		...
		for (const param of func.params) {
			const abiArg = abiFunc?.args.find(
				(a: { name: string }) => a.name === param.name,
			);
			const typeStr =
				includeTypes && abiArg ? `\`${formatType(abiArg.type)}\`` : "-";
	...
	if (func.ok) {
		const returnType =
			includeTypes && abiFunc ? ` (\`${formatType(abiFunc.outputs)}\`)` : "";
...
function generateMapMarkdown(
	...
	if (map.key || map.value || abiMap) {
		...
		const keyType =
			includeTypes && abiMap ? `\`${formatType(abiMap.key)}\`` : "-";
		const valueType =
			includeTypes && abiMap ? `\`${formatType(abiMap.value)}\`` : "-";
