...
export function parseTagLine(line: string, lineNum: number): DocTag | null {
	...
	if (tagName === "prints") {
		...
		const printsWithType = rest.match(
		...
		if (printsWithType) {
			return {
				...
				description:
					`{${printsWithType[1]}} ${(printsWithType[3] || "").trim()}`.trim(),
				line: lineNum,
			};
		}
...
export function extractImplements(tags: DocTag[]): string[] {
	return tags.filter((t) => t.tag === "implements").map((t) => t.description);
}
