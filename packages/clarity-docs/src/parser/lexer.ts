...
export function parseDefine(defineExpr: string): DefineInfo | null {
	...
	const patterns: Array<{ regex: RegExp; type: DefineType }> = [
		...
		{
			regex: /^\(define-public\s+\(([a-zA-Z][a-zA-Z0-9_-]*)\s*/,
			type: "public",
		},
		{
			regex: /^\(define-read-only\s+\(([a-zA-Z][a-zA-Z0-9_-]*)\s*/,
			type: "read-only",
		},
		{
			regex: /^\(define-private\s+\(([a-zA-Z][a-zA-Z0-9_-]*)\s*/,
			type: "private",
		},
		// Variables: (define-data-var NAME type value)
		{
			regex: /^\(define-data-var\s+([a-zA-Z][a-zA-Z0-9_-]*)\s*/,
			type: "data-var",
		},
		...
		{
			regex: /^\(define-constant\s+([a-zA-Z][a-zA-Z0-9_-]*)\s*/,
			type: "constant",
		},
...
export function groupCommentBlocks(
	tokens: Token[],
): Array<{ block: CommentBlock; followingDefine: DefineInfo | null }> {
	const result: Array<{
		block: CommentBlock;
		followingDefine: DefineInfo | null;
	}> = [];
