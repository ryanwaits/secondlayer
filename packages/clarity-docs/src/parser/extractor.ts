...
function isContractHeader(parsed: ReturnType<typeof parseDocBlock>): boolean {
	return parsed.tags.some((t) =>
		(CONTRACT_LEVEL_TAGS as readonly string[]).includes(t.tag),
	);
}
...
export function extractDocs(source: string, sourcePath?: string): ContractDoc {
	...
	for (const { block, followingDefine } of blocks) {
		...
		switch (followingDefine.type) {
			...
			case "private":
				doc.functions.set(
					followingDefine.name,
					buildFunctionDoc(parsed, followingDefine),
				);
				break;
			case "map":
				doc.maps.set(
					followingDefine.name,
					buildMapDoc(parsed, followingDefine),
				);
				break;
			case "data-var":
				doc.variables.set(
					followingDefine.name,
					buildVariableDoc(parsed, followingDefine, "variable"),
				);
				break;
			case "constant":
				doc.constants.set(
					followingDefine.name,
					buildVariableDoc(parsed, followingDefine, "constant"),
				);
				break;
			case "trait":
				doc.traits.set(
					followingDefine.name,
					buildTraitDoc(parsed, followingDefine),
				);
				break;
		}
	}
...
function buildMapDoc(
	...
	return {
		...
		key: keyTag
			? keyTag.name
				? `${keyTag.name} ${keyTag.description}`
				: keyTag.description
			: undefined,
		value: valueTag
			? valueTag.name
...
export async function extractDocsFromFile(
	filePath: string,
): Promise<ContractDoc> {
