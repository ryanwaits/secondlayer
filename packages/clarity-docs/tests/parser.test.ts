...
import { groupCommentBlocks, parseDefine, tokenize } from "../src/parser/lexer";
import {
	extractCaller,
	extractCalls,
	extractErrs,
	...
	describe("parseDefine", () => {
		...
		it("should parse trait", () => {
			const result = parseDefine(
				"(define-trait my-trait ((transfer (uint principal) (response bool uint))))",
			);
			expect(result).toEqual({
				type: "trait",
				name: "my-trait",
...
describe("parser", () => {
				...
				comments: [
					{
						...
						value: "This is just a description",
						line: 1,
					},
				],
				startLine: 1,
		...
		it("should handle multiline descriptions", () => {
			const block = {
				comments: [
					...
					{
						type: "comment" as const,
						value: "continued on second line",
						line: 2,
					},
				],
				startLine: 1,
	...
	describe("parseTagLine - prints", () => {
		it("should parse @prints with type and name", () => {
			const result = parseTagLine(
				"@prints {uint} count The iteration count",
				1,
			);
			expect(result).toEqual({
				tag: "prints",
		...
		it("should parse @prints with tuple type", () => {
			const result = parseTagLine(
				"@prints {from: principal, to: principal} transfer Emitted on success",
				1,
			);
			expect(result).toEqual({
				tag: "prints",
	...
	describe("extractPrints", () => {
		it("should extract prints with name only", () => {
			...
			expect(result).toEqual([
				{ name: "transfer", description: "Emitted on success" },
			]);
		});
	...
	describe("parseTagLine - calls", () => {
		it("should parse @calls with local contract", () => {
			const result = parseTagLine(
				"@calls .token-contract transfer Transfers tokens",
				1,
			);
			expect(result).toEqual({
				tag: "calls",
			...
			const tags = [
				{
					tag: "calls" as const,
					name: "transfer",
					description: ".token Moves tokens",
					line: 1,
				},
			];
	...
	describe("extractImplements", () => {
		...
		it("should extract multiple implements", () => {
			const tags = [
				...
				{
					tag: "implements" as const,
					description: ".burnable.burnable-trait",
					line: 2,
				},
			];
			const result = extractImplements(tags);
...
(define-public (increment (amount uint))
  (ok u1))`;
			...
			expect(func.params).toEqual([
				{ name: "amount", description: "The amount to add" },
			]);
			expect(func.ok).toBe("The new counter value");
			expect(func.errs).toEqual([
				{ code: "ERR_OVERFLOW", description: "Counter would overflow" },
			]);
