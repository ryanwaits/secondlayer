...
const BATCH_SIZE = Number.parseInt(
	process.env.BACKFILL_RAW_TX_BATCH_SIZE || "500",
);
...
async function main() {
  ...
  `.execute(db);
	...
	logger.info("backfill-raw-tx starting", {
		remaining,
	...
	while (true) {
		...
		if (results.size === 0) {
			...
			if (consecutiveEmpty >= 3) {
				logger.warn(
					"3 consecutive batches with 0 fetchable raw_tx — stopping to avoid infinite loop",
					{
      ...
      WHERE tx_id IN (${sql.join(
				matchedTxIds.map((id: string) => sql`${id}`),
				sql`, `,
			)})
    `.execute(db);
        ...
        SET raw_tx = '0x01'
        WHERE tx_id IN (${sql.join(
					unfetchedIds.map((id: string) => sql`${id}`),
					sql`, `,
				)})
      `.execute(db);
			remaining -= unfetched;
