...
import { SBTC_TOKEN_EVENTS_DATASET } from "./schema.ts";

export type SbtcTokenEventsPublisherState = {
	enabled: boolean;
	lastTickAt: number;
...
export const sbtcTokenEventsPublisherState: SbtcTokenEventsPublisherState = {
	enabled: false,
	lastTickAt: 0,
...
export type StartSbtcTokenEventsPublisherOptions = {
	intervalMs?: number;
};

export function startSbtcTokenEventsPublisher(
	options: StartSbtcTokenEventsPublisherOptions = {},
): () => void {
	const enabled = process.env.SBTC_PUBLISHER_ENABLED === "true";
	if (!enabled) {
		logger.info("sBTC token-events publisher disabled");
		return () => {};
	}
	...
	sbtcTokenEventsPublisherState.enabled = true;
	logger.info("Starting sBTC token-events publisher", { intervalMs });
	...
	async function tick() {
		...
		running = true;
		sbtcTokenEventsPublisherState.lastTickAt = Date.now();
		try {
			await publishNextEligibleRange();
			sbtcTokenEventsPublisherState.lastError = null;
		} catch (err) {
			const message = err instanceof Error ? err.message : String(err);
			sbtcTokenEventsPublisherState.lastError = message;
			logger.error("sBTC token-events publisher tick failed", { error: err });
		} finally {
			running = false;
		}
	}
	...
	return () => {
		stopped = true;
		clearInterval(timer);
		sbtcTokenEventsPublisherState.enabled = false;
		logger.info("sBTC token-events publisher stopped");
	};
}
...
export async function publishNextEligibleRange(): Promise<StreamsBulkBlockRange | null> {
	...
	if (tipHeight === null) {
		logger.debug("sBTC token-events publisher: no canonical blocks yet");
		return null;
	}
	...
	if (!range) {
		logger.debug("sBTC token-events publisher: no eligible finalized range", {
			tipHeight,
		});
		return null;
	}
	...
	if (
		...
		logger.debug(
			"sBTC token-events publisher: latest range already published",
			{
				range,
			},
		);
	...
	logger.info("sBTC token-events publisher: exporting range", {
		range,
		tipHeight,
	});
	...
	sbtcTokenEventsPublisherState.lastPublishedRange = result.range;
	sbtcTokenEventsPublisherState.lastPublishedAt = Date.now();
	sbtcTokenEventsPublisherState.publishedTotal += 1;
	logger.info("sBTC token-events publisher: published range", {
		range: result.range,
