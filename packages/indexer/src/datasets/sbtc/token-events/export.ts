...
async function main() {
	...
	const prefix =
		cli.prefix ?? process.env.DATASETS_PREFIX ?? DEFAULT_DATASETS_PREFIX;
	const outputDir =
		cli.outputDir ?? process.env.DATASETS_OUTPUT_DIR ?? "tmp/datasets";
	...
	const range = await resolveRange(cli, rangeSizeBlocks, finalityLagBlocks);
...
function printHelp() {
	console.log(`Usage:
  bun run packages/indexer/src/datasets/sbtc/token-events/export.ts --latest-finalized [--upload] [--force]
  bun run packages/indexer/src/datasets/sbtc/token-events/export.ts --from-block 180000 --to-block 189999 [--upload] [--force]
...
Options:
  ...
  --output-dir <path>      Local output root. Default: tmp/datasets.
  --prefix <path>          Object prefix root. Default: stacks-datasets/mainnet/v0.
