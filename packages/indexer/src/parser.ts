...
async function fetchTxFromApi(txid: string): Promise<{
	...
	try {
		...
		if (!response.ok) {
			logger.debug("Failed to fetch tx from API", {
				txid,
...
export function parseBlock(payload: NewBlockPayload): InsertBlock {
	return {
		...
		index_block_hash: payload.index_block_hash ?? null,
...
export async function parseTransaction(
	...
	if (
		!decoded &&
		tx.txid &&
		!options?.skipApiFallback &&
		TX_DECODE_FALLBACK_ENABLED
	) {
		decoded = await fetchTxFromApi(tx.txid);
		if (decoded) {
			logger.debug("Fetched tx details from API", {
				txid: tx.txid,
				type: decoded.txType,
			});
		}
...
export function parseEvent(
	...
	const eventData =
		(txEvent as unknown as Record<string, unknown>)[type] ?? txEvent;
