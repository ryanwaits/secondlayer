...
import { SubgraphContext, type BlockMeta, type TxMeta } from "../src/runtime/context.ts";
...
describe("context flush statement building", () => {
  test("insert auto-populates _block_height, _tx_id, _created_at", () => {
    const ctx = new SubgraphContext(null as any, "subgraph_test", schema, block, tx);
  ...
  test("column name validation rejects bad names", () => {
    const badSchema: SubgraphSchema = {
      data: {
        columns: {
          ok_col: { type: "text" },
        },
    ...
    const ctx = new SubgraphContext(null as any, "subgraph_test", badSchema, block, tx);
