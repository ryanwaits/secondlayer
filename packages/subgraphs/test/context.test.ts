...
describe("SubgraphContext", () => {
	test("validates table names on insert", () => {
		...
		expect(() => ctx.insert("nonexistent", { sender: "SP1" })).toThrow(
	...
	test("validates table names on update", () => {
		...
		expect(() =>
			ctx.update("nope", { sender: "SP1" }, { amount: 100 }),
		).toThrow('Table "nope" not found');
	});
	...
	test("validates table names on delete", () => {
		...
		expect(() => ctx.delete("bad", { sender: "SP1" })).toThrow(
			'Table "bad" not found',
		);
	});
