...
import { listSubgraphs, updateSubgraphStatus } from "@secondlayer/shared/db/queries/subgraphs";
 ...
 */
async function loadSubgraphDefinition(handlerPath: string): Promise<SubgraphDefinition> {
...
export async function startSubgraphProcessor(opts?: {
  ...
  const stopListening = await listen(CHANNEL_NEW_BLOCK, async () => {
    ...
    const db = getDb();
    const subgraphs = (await listSubgraphs(db)).filter((v: Subgraph) => v.status === "active");
  ...
  // Listen for reorgs
  const stopReorgListening = await listen("subgraph_reorg", async (payload: string | undefined) => {
    if (!running) return;
    try {
