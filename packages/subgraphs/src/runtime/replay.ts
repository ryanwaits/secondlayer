 ...
 *
 * Dedup: replays get a distinct `dedup_key` prefix so they never
 * collide with the original block-time emission. That means running
 * replay twice will produce duplicate outbox rows — the receiver is
 * responsible for idempotency via the `webhook-id` header.
 */
