...
/**
 * Handlers keyed by source name, one per source. Each handler's `event` is
 * typed from the matching source's filter `type` (e.g. a `print_event` source
 * gives `event.topic: string`), and `ctx` is typed against the subgraph
 * `schema` (table names + row columns checked).
 */
export type TypedHandlers<
	Sources extends Record<string, SubgraphFilter>,
	S extends SubgraphSchema,
> = { [K in keyof Sources]-?: HandlerFn<Sources[K], S> };
