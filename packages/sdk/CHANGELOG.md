...
- c62de3e: `streamsClient.events.consume()` takes an optional `wake: () => Promise<void>` — on an empty page, the loop races the usual `emptyBackoffMs` sleep against it instead of always sleeping the full backoff. A `wake` that never resolves, rejects, or isn't supplied degrades to exactly the old polling behavior.
...
- 5066016: Reindex no longer takes a block range on the SDK client or the MCP tool.
...
- eb8dafc: Sprint of filter-surface convergence, without unifying what should not unify (see the new ADR, docs/internal/charter/index-vs-streams.md).
...
- Updated dependencies [eb8dafc]
  - @secondlayer/stacks@3.2.0

## 6.46.1

### Patch Changes

- 0773b4c: Two silent-wrong-results fixes from the filter-surfaces audit.
...
- 50211c2: Field projection on the transfer feeds. `sl.index.ftTransfers.list({ fields: ["amount", "sender"] })` (and `nftTransfers`) returns rows carrying only those columns plus `cursor`/`block_height`/`event_type`, the row type narrows to match, and `walk` and the callable shorthand narrow identically. Omitting `block_time` also lets the server skip the blocks join, the same win `/v1/index/events` already had.
...
- b0ccb0b: Field projection now covers sBTC withdrawals, transactions, and pox-5 events alongside deposits and index events. Each narrows its row type to the requested columns, so reading an unrequested one is a compile error.
...
- Updated dependencies [b0ccb0b]
  - @secondlayer/subgraphs@3.23.2

## 6.43.0

### Minor Changes

- f15de25: Field projection on sBTC deposit reads. `sl.sbtc.deposits.list({ fields: ["amount", "bitcoin_txid"] })` returns rows carrying only those columns plus `cursor`/`block_height`, and the row type narrows to match — reading an unrequested column is a compile error rather than `undefined` at runtime.
...
- 1294284: New `@secondlayer/sdk/streams/rows` subpath for the per-type guard + decode pairs that return **database row** shapes (`decoded_payload` nested, `source_cursor` carried), as distinct from the flat API rows `decode()` returns.
...
- Updated dependencies [1294284]
  - @secondlayer/shared@7.4.0

## 6.41.0

### Minor Changes

- 4369792: **Breaking:** `on` is now the chain-event filter union at the root of `@secondlayer/stacks`, not a second set of factories.
...
- 114ba4c: MCP: `codegen_index_schema` generates an ORM schema (kysely/prisma/drizzle/json-schema) for the Index tables without leaving the conversation — the same generator `sl codegen index` calls, so output is identical either way.
...
- 9377a35: Index reads take a `fields` selection, and the returned row type narrows to match.
...
- 4a12426: Sinks: the consume loops can now own the three hard parts of a durable indexer — checkpoint persistence, rows+cursor atomicity, and reorg rollback — instead of teaching them in doc comments.
...
- Updated dependencies [39ba91e]
  - @secondlayer/stacks@2.20.1
...
- 2f4a950: One error family, and consume loops that survive transient failures:
...
- Updated dependencies [025779e]
  - @secondlayer/stacks@2.19.2

## 6.36.0

### Minor Changes

- 3abd001: Consumer and client guardrails:
