...
- d2889e1: Add `@secondlayer/stacks/simnet`: a `simnet()` transport so `getContract` and public/wallet actions run against Clarinet simnet. Optional peer of that entry only — the root SDK still does not load clarinet-sdk.
...
- 0fff1ab: Three filter-union fixes from the cross-surface audit: renames stop masquerading as capability gaps, and one runtime 400 becomes a projection-time throw.
...
- 12d1d6b: Subgraph handlers can read contract state. `readContractAt(ctx, contractId, abi, options)` returns the ABI's camelCased read-only methods — same shape as `getContract`, minus `call`, since a handler indexes the chain and never writes to it.
...
- b4930cd: New `@secondlayer/stacks/filters` subpath — the canonical chain-event filter vocabulary. One chain event was spelled five different ways across Index, Streams, Subgraphs, and Subscriptions (with three incompatible `minAmount` types among them); `on.*` is now the single spelling, projected explicitly to each surface:
...
- 2586ebe: PoX-5 Bitcoin Staking (SIP-045) — new `@secondlayer/stacks/pox5` module, pinned against the final contract in stacks-core 4.0.0 (Epoch 4.0 activates at Bitcoin block 960,230, ~2026-07-29).
...
- 837dfc5: Bitcoin address derivation + network-aware sBTC signers address.
