...
describe("ABI Type Definition Guards", () => {
	...
	it("should identify response types", () => {
		expect(isAbiResponse({ response: { ok: "bool", error: "uint128" } })).toBe(
			true,
		);
...
describe("isTraitReference", () => {
	it("should validate trait references (contract principals)", () => {
		expect(
			isTraitReference("SP2J6ZY48GV1EZ5V2V5RB9MP66SW86PYKKNRV9EJ7.my-trait"),
		).toBe(true);
		expect(
			isTraitReference("ST2CY5V39NHDPWSXMW9QDT3HC3GD6Q6XX4CFRK9AG.test-trait"),
		).toBe(true);
	});

	it("should reject standard principals", () => {
		expect(isTraitReference("SP2J6ZY48GV1EZ5V2V5RB9MP66SW86PYKKNRV9EJ7")).toBe(
			false,
		);
	});
