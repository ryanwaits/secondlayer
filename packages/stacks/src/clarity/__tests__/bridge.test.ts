...
describe("clarityValueToJS", () => {
	...
	it("should convert response err", () => {
		const type: AbiType = { response: { ok: "bool", error: "uint128" } };
		expect(clarityValueToJS(type, Cl.error(Cl.uint(100n)))).toEqual({
			err: 100n,
		});
	});
});
...
describe("roundtrip", () => {
	...
	it("should roundtrip strings", () => {
		...
		expect(
			clarityValueToJS(asciiType, jsToClarityValue(asciiType, "hello")),
		).toBe("hello");
		expect(
			clarityValueToJS(utf8Type, jsToClarityValue(utf8Type, "hello 🌍")),
		).toBe("hello 🌍");
	});
