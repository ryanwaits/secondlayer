...
import { POX_ADDRESS_VERSION, POX_CONTRACTS } from "../constants.ts";
	...
	describe("Utils", () => {
		...
		describe("cycle calculations", () => {
			...
			test("burnHeightToRewardCycle", () => {
				expect(burnHeightToRewardCycle(666050n, firstHeight, cycleLength)).toBe(
					0n,
				);
				expect(burnHeightToRewardCycle(668150n, firstHeight, cycleLength)).toBe(
					1n,
				);
				expect(burnHeightToRewardCycle(670250n, firstHeight, cycleLength)).toBe(
					2n,
				);
			});
