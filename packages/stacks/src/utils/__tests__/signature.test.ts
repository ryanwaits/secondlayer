...
const PUBLIC_KEY =
	"0290255f88fa311f5dee9425ce33d7d516c24157e2aae8e25a6c631dd6f7322aef";
const MESSAGE_HASH =
...
describe("parseSignature", () => {
	...
	test("throws on invalid length", () => {
		expect(() => parseSignature("deadbeef")).toThrow(
...
describe("VRS / RSV conversion", () => {
	...
	test("roundtrip", () => {
		expect(signatureRsvToVrs(signatureVrsToRsv(SIGNATURE_VRS))).toBe(
			SIGNATURE_VRS,
		);
		expect(signatureVrsToRsv(signatureRsvToVrs(SIGNATURE_RSV))).toBe(
			SIGNATURE_RSV,
		);
	});
});
