...
describe("request", () => {
	...
	test("calls provider with correct method and params", async () => {
		const mockRequest = mock(() => Promise.resolve({ txId: "0x123" }));
	...
	test("caches addresses on stx_getAddresses", async () => {
		...
		setProvider({
			request: mock(() => Promise.resolve({ addresses })),
		});
	...
	test("does not cache on non-address methods", async () => {
		setProvider({
			request: mock(() => Promise.resolve({ txId: "0x123" })),
		});
	...
	test("wraps generic errors in ConnectError", async () => {
		setProvider({
			request: mock(() => Promise.reject(new Error("network down"))),
		});
