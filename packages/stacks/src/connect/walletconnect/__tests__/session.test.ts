			...
			if (rpc.method && rpc.id != null && !this.respondedIds.has(rpc.id)) {
				...
				queueMicrotask(() => {
					...
					this.simulateMessage(
						JSON.stringify({ id: rpc.id, jsonrpc: "2.0", result }),
					);
				});
			}
		} catch {}
...
describe("WcSession", () => {
	test("pair() returns valid WC URI", async () => {
		...
		expect(uri).toMatch(
			/^wc:[0-9a-f]{64}@2\?relay-protocol=irn&symKey=[0-9a-f]{64}$/,
		);
	...
	test("request throws without active session", async () => {
		const session = new WcSession(TEST_CONFIG);
		await expect(session.request("stx_getAddresses")).rejects.toThrow(
			"No active WC session",
		);
	});
});
