...
describe("BNS Extension", () => {
	describe("Utils", () => {
		...
		test("parseFQN throws on invalid format", () => {
			expect(() => parseFQN("alice.bob.btc")).toThrow(
	...
	describe("Constants", () => {
		test("mainnet contract address is correct", () => {
			expect(BNS_CONTRACTS.mainnet.address).toBe(
		...
		test("testnet contract address is correct", () => {
			expect(BNS_CONTRACTS.testnet.address).toBe(
		...
		test("zonefile resolver mainnet address is correct", () => {
			...
			expect(ZONEFILE_RESOLVER_CONTRACTS.mainnet.name).toBe(
				"zonefile-resolver",
			);
		});
		...
		test("zonefile resolver testnet address is correct", () => {
			...
			expect(ZONEFILE_RESOLVER_CONTRACTS.testnet.name).toBe(
				"zonefile-resolver",
			);
		});
	});
	...
	describe("Type Safety", () => {
		test("TypeScript types are exported", () => {
			...
			void (client.bns.getPrimaryName as (
				address: string,
			) => Promise<string | null>);
