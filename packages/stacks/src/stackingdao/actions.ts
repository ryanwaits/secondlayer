...
export async function withdrawIdle(
	client: Client,
	params: WithdrawIdleParams,
): Promise<string> {
	const core = getCoreContract(client);
	const ststx = STACKINGDAO_CONTRACTS.ststxToken;
...
export async function getStSTXBalance(
	client: Client,
	address: string,
): Promise<bigint> {
