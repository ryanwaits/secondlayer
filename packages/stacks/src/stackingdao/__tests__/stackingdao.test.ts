...
describe("StackingDAO Extension", () => {
	describe("Constants", () => {
		...
		test("trait contracts are set", () => {
			...
			expect(TRAIT_CONTRACTS.directHelpers).toBe(
				`${DEPLOYER}.direct-helpers-v4`,
			);
	...
	describe("Type Safety", () => {
		test("types compile correctly", () => {
			...
			void (client.stackingDao.deposit as (params: {
				amount: bigint;
			}) => Promise<string>);
			void (client.stackingDao.getStSTXBalance as (
				address: string,
			) => Promise<bigint>);
			void (client.stackingDao.getFees as () => Promise<{
				stackFee: bigint;
				unstackFee: bigint;
