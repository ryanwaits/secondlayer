...
describe("multi-sig", () => {
	...
	describe("signMultiSigWithAccount", () => {
		test("account signing matches private key signing", async () => {
			...
			const withAccount = await signMultiSigWithAccount(
				tx,
				account1,
				publicKeys,
			);

			const keyCond = withKey.auth
				.spendingCondition as MultiSigSpendingCondition;
	...
	describe("non-sequential multi-sig (SIP-027)", () => {
		...
		test("combineMultiSigSignatures errors on sequential hash mode", () => {
			...
			expect(() => combineMultiSigSignatures(seqTx, [])).toThrow(
				"non-sequential",
			);
		});
		...
		test("combineMultiSigSignatures errors with no signatures", () => {
			const tx = buildNonSeqTx();
			expect(() => combineMultiSigSignatures(tx, [tx])).toThrow(
				"No signatures",
			);
		});
		...
		test("combine and pass-through produce same signatures", () => {
			...
			const ptCond = passThrough.auth
				.spendingCondition as MultiSigSpendingCondition;
			const cbCond = combined.auth
				.spendingCondition as MultiSigSpendingCondition;
