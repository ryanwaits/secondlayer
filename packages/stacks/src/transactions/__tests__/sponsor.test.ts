...
const SPONSOR_KEY =
	"9888882b59abc471d140e5e0bb9c9109ae1ccf0a23b18fe3d20e3b5e12fb02e2";
...
describe("sponsored transactions", () => {
	...
	test("signTransaction + signSponsor full flow with private keys", () => {
		...
		const originCond = fullySigned.auth
			.spendingCondition as SingleSigSpendingCondition;
		const sponsorCond = (fullySigned.auth as SponsoredAuthorization)
			.sponsorSpendingCondition as SingleSigSpendingCondition;
	...
	test("signTransactionWithAccount + signSponsorWithAccount flow", async () => {
		...
		const fullySigned = await signSponsorWithAccount(
			withSponsor,
			sponsorAccount,
		);
	...
	test("serialization roundtrip preserves sponsored tx", () => {
		...
		const origCond = deserialized.auth
			.spendingCondition as SingleSigSpendingCondition;
		const origCondExpected = fullySigned.auth
			.spendingCondition as SingleSigSpendingCondition;
		expect(origCond.signature).toBe(origCondExpected.signature);

		// Sponsor signature preserved
		const sponsCond = (deserialized.auth as SponsoredAuthorization)
			.sponsorSpendingCondition as SingleSigSpendingCondition;
		const sponsCondExpected = (fullySigned.auth as SponsoredAuthorization)
			.sponsorSpendingCondition as SingleSigSpendingCondition;
	...
	test("private key signing matches account signing for origin", async () => {
		...
		const signedWithAccount = await signTransactionWithAccount(
			tx,
			originAccount,
		);

		const keySig = (
			signedWithKey.auth.spendingCondition as SingleSigSpendingCondition
		).signature;
		const accountSig = (
			signedWithAccount.auth.spendingCondition as SingleSigSpendingCondition
		).signature;
	...
	test("private key signing matches account signing for sponsor", async () => {
		...
		const signedWithAccount = await signSponsorWithAccount(
			withSponsor,
			sponsorAccount,
		);

		const keySig = (
			(signedWithKey.auth as SponsoredAuthorization)
				.sponsorSpendingCondition as SingleSigSpendingCondition
		).signature;
		const accountSig = (
			(signedWithAccount.auth as SponsoredAuthorization)
				.sponsorSpendingCondition as SingleSigSpendingCondition
		).signature;
		expect(keySig).toBe(accountSig);
	});
