...
- 00c009b: Remove the deprecated tool aliases and rename `subgraphs_get`, so every tool name is `<product>_<verb>` with no second spelling.
...
- 9063c6d: Prune the MCP tool surface to the golden path (32 tools). The MCP server is a distribution channel for agents, not a parity mirror — periphery surfaces stay available via REST `/v1` + OpenAPI.
