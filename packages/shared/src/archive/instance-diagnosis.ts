/**
 * One reading of `/status` (or `/public/status`), shared by the CLI
 * (`status`, `doctor`, `context`) and the SDK (`sl.instance.diagnose`).
 *
 * These surfaces used to each interpret the same payload on their own and
...
/** The subset of `/status` a diagnosis reads. Everything is optional:
...
/**
 * Turn a `/status` payload into a verdict plus zero or more issues.
