...
import { registerSubgraph, getSubgraph, listSubgraphs, updateSubgraphStatus, deleteSubgraph } from "../src/db/queries/subgraphs.ts";
...
describe.skipIf(SKIP)("Subgraphs Queries", () => {
  const testDef = {
    name: "test-subgraph",
    version: "1.0.0",
    definition: { name: "test-subgraph", sources: [{ contract: "SP::c" }], schema: {} },
    schemaHash: "abc123",
  ...
  afterEach(async () => {
    ...
    await sql.raw("DROP SCHEMA IF EXISTS subgraph_test_subgraph CASCADE").execute(db);
  });
  ...
  test("registerSubgraph upserts on conflict", async () => {
    ...
    const updated = await registerSubgraph(db, { ...testDef, schemaHash: "def456", version: "2.0.0" });
  ...
  test("deleteSubgraph removes subgraph and drops schema", async () => {
    ...
    await sql.raw("CREATE SCHEMA IF NOT EXISTS subgraph_test_subgraph").execute(db);
