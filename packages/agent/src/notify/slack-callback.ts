...
export async function handleSlackCallback(req: Request, deps: CallbackDeps): Promise<Response> {
  ...
  if (payload.type === "url_verification") {
    return Response.json({ challenge: (payload as unknown as { challenge: string }).challenge });
  }
    ...
    try {
      switch (action.action_id) {
        case "agent_dismiss": {
          ...
          if (messageTs) {
            await slack.updateMessage(messageTs, addResolvedFooter(originalBlocks, `Dismissed by @${user}`));
          }
        ...
        case "agent_restart": {
          ...
          const result = await executor.execute("restart_service", parsed.service, match);
          resolveAlert(db, parsed.alertId);

          if (messageTs) {
            await slack.postThreadReply(messageTs, `Restart triggered by @${user}: ${result.outcome} — ${result.detail}`);
            await slack.updateMessage(messageTs, addResolvedFooter(originalBlocks, `Restarted by @${user}`));
          }
        ...
        case "agent_investigate": {
          if (messageTs) {
            await slack.postThreadReply(messageTs, `:mag: Investigating... (triggered by @${user})`);
          }
          ...
          if (messageTs) {
            const diagBlocks = buildDiagnosisBlocks(
              { ...diagnosis, suggestedAction: diagnosis.suggestedAction ?? undefined },
              parsed.alertId,
              parsed.service
            );
        ...
        case "agent_verify": {
          if (messageTs) {
            await slack.postThreadReply(messageTs, `:mag: Running health check... (triggered by @${user})`);
          }
          ...
          const summary = Object.entries(health)
            .map(([k, v]) => `${k}: ${v && typeof v === "object" && "ok" in v ? (v.ok ? ":white_check_mark:" : ":x:") : ":question:"}`)
            .join("\n");

          if (messageTs) {
            await slack.postThreadReply(messageTs, `*Health check results:*\n${summary}`);
          }
        ...
        case "agent_execute_suggested": {
          ...
          const result = await executor.execute(execAction as PatternMatch["action"], parsed.service, match);
          resolveAlert(db, parsed.alertId);

          if (messageTs) {
            await slack.postThreadReply(messageTs, `Executed \`${execAction}\` by @${user}: ${result.outcome} — ${result.detail}`);
            await slack.updateMessage(messageTs, addResolvedFooter(originalBlocks, `Executed ${execAction} by @${user}`));
          }
          log(`Alert #${parsed.alertId} execute ${execAction} by ${user}: ${result.outcome}`);
          break;
        }
