...
export async function pollHealth(): Promise<HealthStatus> {
	...
	const stacksNodeHost =
		process.env.STACKS_NODE_URL ?? `${nodeServerUrl}:20443`;
	...
	const [indexer, api, stacksNode, integrity] = await Promise.allSettled([
		...
		fetchJson<{ stacks_tip_height?: number; burn_block_height?: number }>(
			`${stacksNodeHost}/v2/info`,
		),
		fetchJson<{ gaps?: number; totalMissing?: number }>(
			`${indexerHost}/health/integrity`,
		),
	]);
	...
	if (stacksNode.status === "fulfilled") {
		...
		if (isNetworkError) {
			console.warn(
				`[health-poller] Node server unreachable at ${stacksNodeHost}: ${reason}`,
			);
	...
	return {
		indexer:
			indexer.status === "fulfilled"
				...
				: {
						ok: false,
						error: String((indexer as PromiseRejectedResult).reason),
					},
		api:
		...
		integrity:
			integrity.status === "fulfilled"
				? {
						ok: true,
						gaps: integrity.value.gaps,
						totalMissing: integrity.value.totalMissing,
					}
				: {
						ok: false,
						error: String((integrity as PromiseRejectedResult).reason),
					},
	};
}
...
export async function collectSystemMetrics(): Promise<SystemMetrics> {
	...
	if (dfLines.length >= 2) {
		...
		diskUsedPct = Number.parseInt(parts[4]?.replace("%", "") ?? "0");
		diskAvailBytes = Number.parseInt(parts[3] ?? "0");
	}
	...
	if (freeLines.length >= 2) {
		...
		const total = Number.parseInt(parts[1] ?? "1");
		const avail = Number.parseInt(parts[6] ?? "0");
		memAvailBytes = avail;
		memUsedPct = Math.round(((total - avail) / total) * 100);
	}

	// Docker stats
	const statsResult = Bun.spawnSync([
		"docker",
		"stats",
		"--no-stream",
		"--format",
	...
	for (const line of statsResult.stdout.toString().trim().split("\n")) {
		...
		try {
			...
			const restartCount = Number.parseInt(inspectParts[0]) || 0;
...
export function detectAnomalies(
	...
	if (
		...
		current.health.indexer.lastSeenHeight ===
			previous.health.indexer.lastSeenHeight
	) {
		...
		const nodeStillSyncing =
			nodeTip &&
			prevNodeTip &&
			nodeTip !== prevNodeTip &&
			nodeTip > (current.health.indexer.lastSeenHeight ?? 0);
	...
	if (
		...
		(current.health.integrity.totalMissing ?? 0) >
			(previous.health.integrity.totalMissing ?? 0)
	) {
		anomalies.push({
			name: "gap_increase",
			severity: "warn",
	...
	const containerByService = new Map(
		current.metrics.containers.map((c) => [
			c.name.replace(/^secondlayer-|-1$/g, ""),
			c,
		]),
	);
	...
	for (const { ok, error, svc } of [
		{
			ok: current.health.indexer.ok,
			error: current.health.indexer.error,
			svc: "indexer",
		},
		...
		const recentlyRestarted =
			container?.startedAt && now - container.startedAt < RECENT_RESTART_MS;
