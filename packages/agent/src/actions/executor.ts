...
export class ActionExecutor {
  ...
  async execute(action: ActionType, service: string, context: PatternMatch): Promise<ExecuteResult> {
    // Log decision
    const tier = "t1_auto";
    ...
    if (action === "restart_service") {
      if ((NEVER_RESTART as readonly string[]).includes(service)) {
        const result = { outcome: "blocked", detail: `${service} is in NEVER_RESTART list` };
        insertDecision(this.db, { tier, trigger: context.name, analysis: context.message, action, service, outcome: result.outcome, costUsd: 0 });
        log(`BLOCKED: restart ${service} — ${result.detail}`);
        return result;
      }

      if ((WARN_RESTART as readonly string[]).includes(service)) {
        const result = { outcome: "alert_only", detail: `${service} is in WARN_RESTART list — alerting instead` };
        insertDecision(this.db, { tier, trigger: context.name, analysis: context.message, action: "alert_only", service, outcome: result.outcome, costUsd: 0 });
        log(`WARN: ${service} restart requested but deferred to alert`);
        return result;
      }

      if (checkCooldown(this.db, service, "restart_service", this.config.maxRestartsPerHour)) {
        const result = { outcome: "cooldown", detail: `${service} restart rate limited (max ${this.config.maxRestartsPerHour}/hr)` };
        insertDecision(this.db, { tier, trigger: context.name, analysis: context.message, action, service, outcome: result.outcome, costUsd: 0 });
    ...
    if (this.config.dryRun) {
      const result = { outcome: "dry_run", detail: `Would execute ${action} on ${service}` };
      insertDecision(this.db, { tier, trigger: context.name, analysis: context.message, action, service, outcome: result.outcome, costUsd: 0 });
    ...
    switch (action) {
      ...
      case "escalate":
      case "none":
        result = { outcome: action, detail: `No automated action for ${action}` };
        break;
      default:
  ...
  private async restartService(service: string): Promise<ExecuteResult> {
    ...
    const result = Bun.spawnSync([...this.config.composeCmd, "restart", svcName]);
