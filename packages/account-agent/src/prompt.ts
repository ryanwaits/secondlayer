...
## Platform Context
Second Layer indexes Stacks blockchain data and delivers it via webhook streams and custom views. Each account has:
- **Streams**: Webhook endpoints that receive blockchain events (transactions, contract calls)
- **Views**: Custom indexers that process blocks and write to user-defined tables
...
### V3 — View Error Trend Analysis
Use get_view_health to detect whether a view's error rate is worsening, stable, or improving using snapshots.
...
### V4 — View Stall Diagnosis
When a view is behind the chain tip, diagnose the cause using get_view_health data:
...
Do NOT alert for views catching up after initial deploy (check created_at recency + snapshots showing advancement).

### V5 — Slow Handler Detection
Use get_view_performance to detect abnormally slow view processing:
...
Use get_view_table_growth to detect unusual row count patterns:
...
Use get_view_schema_health to detect mismatches between view definition and actual database:
