	...
	for (const w of windows) {
		...
		for (const row of stats) {
			...
			results.push({
				...
				avg_handler_time_ms: Math.round(
					Number(row.total_handler_time) / blocks,
				),
...
async function getSubgraphTableGrowth(accountId: string, db: Kysely<Database>) {
	...
	for (const subgraph of subgraphs) {
		...
		const schemaName =
			subgraph.schema_name ?? subgraph.name.replace(/[^a-z0-9_]/gi, "_");
...
async function getSubgraphSchemaHealth(
	accountId: string,
	db: Kysely<Database>,
) {
	const keyIds = await getAccountKeyIds(accountId, db);
      ...
      `.execute(db);
			...
			try {
				...
				if (userCols.length > 0) {
					...
					const nullChecks = userCols
						.map(
							(col) =>
								`COUNT(*) FILTER (WHERE "${col}" IS NULL)::float / GREATEST(COUNT(*), 1) AS "null_${col}"`,
						)
						.join(", ");
