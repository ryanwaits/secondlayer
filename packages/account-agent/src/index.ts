...
const BUDGET_USD = parseFloat(process.env.ACCOUNT_AGENT_BUDGET_USD || "0.25");
  ...
  try {
    ...
    for (let turn = 0; turn < 10; turn++) {
      ...
      if (response.stop_reason === "end_turn") {
        insightsJson =
          textBlocks.map((b) => b.text).join("") || null;
        break;
      }
    }
    ...
    for (const insight of insights) {
      ...
      const active = await db
        ...
        .where("resource_id", insight.resource_id ? "=" : "is", insight.resource_id)
      ...
      const recentlyDismissed = !active
        ? await db
            ...
            .where("dismissed_at", ">", new Date(Date.now() - DISMISS_COOLDOWN_MS))
            .executeTakeFirst()
        : null;
