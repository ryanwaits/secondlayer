...
function normalizeRow(row: NftTransferRow): NftTransferEvent {
	return {
		cursor: row.cursor,
...
export async function readNftTransfers(
	...
	if (params.assetIdentifier) {
		predicates.push(sql`asset_identifier = ${params.assetIdentifier}`);
	}
	if (params.sender) {
	...
	const orderBy = params.contractId
		...
		: params.assetIdentifier
			? sql`asset_identifier ASC, block_height ASC, event_index ASC`
			: params.sender
