...
describe("createExtendedApp", () => {
	...
	test("createApiApp 404s /extended/v1/tx/.../events with v1 envelope", async () => {
		const app = createApiApp("oss");
	...
	test("createApiApp 404s /extended/v1/address/.../stx with v1 envelope", async () => {
		const app = createApiApp("oss");
	...
	test("createApiApp 404s /extended/v1/names with v1 envelope", async () => {
		const app = createApiApp("oss");
		const res = await app.request("/extended/v1/names?address=SP1");
		expect(res.status).toBe(404);
		const body = (await res.json()) as { code: string };
		expect(body.code).toBe("NOT_FOUND");
	});
