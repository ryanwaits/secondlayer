import type { ColumnType, ViewColumn, ViewTable } from "./types.ts";
...
export type InferColumnType<C extends ViewColumn> =
...
export type InferTableRow<T extends ViewTable> = SystemRow & {
...
export interface ViewTableClient<TRow> {
  findMany(options?: FindManyOptions<TRow>): Promise<TRow[]>;
  count(where?: WhereInput<TRow> & SystemWhereAliases): Promise<number>;
}

// ── Full view client inference ───────────────────────────────────────────

/** Infer a typed client object from a view definition shape */
export type InferViewClient<T> = T extends { schema: infer S }
  ? {
      [K in keyof S]: S[K] extends ViewTable ? ViewTableClient<InferTableRow<S[K]>> : never;
    }
  : never;
