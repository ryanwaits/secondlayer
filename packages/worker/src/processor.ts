import { StreamNotFoundError, logger } from "@secondlayer/shared";
import { getDb } from "@secondlayer/shared/db";
import type { Block, Job, Stream } from "@secondlayer/shared/db";
...
import { type MatchResult, evaluateFilters } from "./matcher/index.ts";
...
export async function processJob(job: Job): Promise<ProcessedJob> {
	...
	if (result.success) {
		...
		if (recentFailures >= MAX_CONSECUTIVE_FAILURES) {
			await disableStreamOnFailure(
				stream.id,
				result.error || "Too many delivery failures",
			);
		}
 ...
 */
async function disableStreamOnFailure(
	streamId: string,
	error: string,
): Promise<void> {
	const db = getDb();
