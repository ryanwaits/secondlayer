...
import { matchFtBurn, matchFtMint, matchFtTransfer } from "./ft.ts";
import { matchNftBurn, matchNftMint, matchNftTransfer } from "./nft.ts";
import {
	matchStxBurn,
	matchStxLock,
	matchStxMint,
...
function evaluateFilter(
	...
	switch (filter.type) {
		...
		case "contract_call":
			return {
				transactions: matchContractCall(filter, transactions),
				events: [],
			};
		case "contract_deploy":
			return {
				transactions: matchContractDeploy(filter, transactions),
				events: [],
			};
		case "print_event":
