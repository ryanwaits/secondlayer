...
import { dim, error, formatKeyValue, info, success } from "../lib/output.ts";
...
export function registerReplayCommand(program: Command): void {
	program
		...
		.option(
			"--fixture <path>",
			"Load block from fixture file (only with --block)",
		)
		.action(
			async (
				rawStreamId: string,
				...
				try {
					...
					if (options.last) {
						...
						info(
							`Replaying last ${lastCount} blocks (${fromBlock} to ${toBlock})`,
						);
						...
						console.log(dim("\nExamples:"));
						console.log(
							dim("  sl streams replay <id> --from 100000 --to 100500"),
						);
						console.log(dim("  sl streams replay <id> --last 1000"));
						console.log(dim("  sl streams replay <id> --block 100000"));
						console.log(
							dim(
								"  sl streams replay <id> --block 100000 --fixture block.json",
							),
						);
						process.exit(1);
					}
					...
					const response = await fetch(
						`${resolveApiUrl(config)}/api/streams/${streamId}/replay`,
						{
							method: "POST",
							headers: authHeaders(config),
					...
					console.log(
						dim("Use 'sl streams logs <id> -f' to monitor progress."),
					);
				} catch (err) {
...
async function triggerBlock(
	...
	const response = await fetch(
		`${resolveApiUrl(config)}/api/streams/${streamId}/trigger`,
		{
			method: "POST",
			headers: authHeaders(config),
