...
import { isProcessRunning, loadDevState } from "../lib/dev-state.ts";
import {
	blue,
	dim,
	error,
...
async function showBlocks(limit: number, json: boolean): Promise<void> {
	try {
		...
		console.log("");
		console.log(
			dim("  HEIGHT".padEnd(10)) + dim("HASH".padEnd(24)) + dim("TIME"),
		);
...
async function showGaps(limit: number, json: boolean): Promise<void> {
	try {
		...
		if (json) {
			console.log(
				JSON.stringify({ gaps, totalMissingBlocks: missing }, null, 2),
			);
			process.exit(0);
		}
		...
		console.log("");
		console.log(
			dim("  GAP_START".padEnd(14)) + dim("GAP_END".padEnd(14)) + dim("SIZE"),
		);
		...
		console.log("");
		console.log(
			`${yellow(gaps.length.toString())} gaps, ${red(missing.toString())} missing blocks`,
		);
		console.log("");
		process.exit(0);
	} catch (err) {
...
async function resyncDatabase(
	...
	try {
		...
		console.log(
			`  1. Delete all indexed data (${red(blockCount.toString())} blocks)`,
		);
		...
		if (
			...
			isProcessRunning(state.services.indexer.pid)
		) {
		...
		if (backfill) {
			...
			for (let i = 1; i <= tip; i += concurrency) {
				...
				process.stdout.write(
					`\r  ${green(`${fetched}`)} fetched, ${errors > 0 ? red(`${errors} errors`) : "0 errors"} (${pct}%)`,
				);
			}
