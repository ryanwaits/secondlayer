...
export function registerSyncCommand(program: Command): void {
	program
		...
		.option(
			"--concurrency <n>",
			"Parallel fetch limit (default: 1 for hiro, 5 for node)",
		)
		.option(
			"--delay <ms>",
			"Delay between batches in ms (default: 500 for hiro, 0 for node)",
		)
		...
		.action(async function (this: Command) {
			...
			const indexerUrl =
				process.env.INDEXER_URL || `http://localhost:${config.ports.indexer}`;
			...
			try {
				...
				if (opts.gaps) {
					...
					ranges = gaps.map((g: { gapStart: number; gapEnd: number }) => ({
						start: g.gapStart,
						end: g.gapEnd,
					}));
				...
				if (!opts.yes) {
					...
					const sourceLabel = useLocal
						...
						: useHiro
							...
							: "local node";
					console.log(
						`This will fetch ${yellow(totalBlocks.toString())} blocks from ${sourceLabel}.`,
					);
					if (useHiro) {
						console.log(
							dim(
								"Note: Hiro API fetches are slower due to per-tx event lookups.",
							),
						);
					}
				...
				if (useHiro && batchDelay > 0) {
					info(
						`Pacing: ${batchDelay}ms delay between batches (concurrency: ${concurrency})`,
					);
				}
					...
					for (let i = 0; i < heights.length; i += concurrency) {
						...
						const results = await Promise.allSettled(
							batch.map(async (height) => {
								...
								if (!res.ok) {
									throw new Error(
										`Indexer rejected block ${height}: ${res.status}`,
									);
								}
						...
						process.stdout.write(
							`\r  ${green(`${fetched}`)} fetched, ${errors > 0 ? red(`${errors} errors`) : "0 errors"} (${pct}%) ${dim(`${rate.toFixed(1)} blk/s — ETA ${etaStr}`)}   `,
						);
