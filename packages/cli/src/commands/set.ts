...
import { dim, error, info, red, success, warn } from "../lib/output.ts";
...
export function registerSetCommand(program: Command): void {
	program
		...
		.option(
			"-o, --option <kv...>",
			"Set stream option (key=value, e.g. maxRetries=5)",
		)
		.action(
			async (
				id: string | undefined,
				...
				try {
					...
					if (options.option?.length) {
						...
						success(
							`Updated stream options: ${Object.entries(parsedOptions)
								.map(([k, v]) => `${k}=${v}`)
								.join(", ")}`,
						);
						if (!state) return;
...
async function setSingleStream(id: string, state: string): Promise<void> {
	switch (state) {
		...
		default:
			error(
				`Unknown state: ${state}. Use active, disabled, or paused (with --all)`,
			);
			process.exit(1);
	}
}
...
async function setAllStreams(state: string, wait?: boolean): Promise<void> {
	switch (state) {
		...
		case "active": {
			...
			success(
				`Resumed ${result.resumed} stream${result.resumed === 1 ? "" : "s"}`,
			);
			break;
		}
		default:
...
async function retryStream(id: string, replayFailed?: boolean): Promise<void> {
	...
	const stream = (await getRes.json()) as {
		id: string;
		name: string;
		status: string;
