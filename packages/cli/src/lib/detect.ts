...
export async function detectRunningContainers(): Promise<NodeInfo | null> {
	try {
		...
		const psResult = await Bun.$`docker ps --format "{{.Names}}"`
			.quiet()
			.nothrow();
		...
		const inspectResult = await Bun.$`docker inspect ${stacksContainer}`
			.quiet()
			.nothrow();
 ...
 */
async function extractPathFromMounts(container: {
	Mounts?: Mount[];
	...
	if (binds && Array.isArray(binds)) {
		for (const bind of binds) {
			...
			if (
				source &&
				(source.includes("stacks-blockchain") || source.includes("stacks-node"))
			) {
 ...
 */
async function findDockerComposeRoot(
	startPath: string,
): Promise<string | null> {
...
function detectNetworkFromContainer(container: {
	...
	if (config?.Env) {
		for (const env of config.Env) {
			...
			if (
				lower.includes("network=testnet") ||
				lower.includes("stacks_network=testnet")
			) {
				return "testnet";
			}
			if (
				lower.includes("network=mainnet") ||
				lower.includes("stacks_network=mainnet")
			) {
				return "mainnet";
			}
		}
...
async function expandGlobPattern(pattern: string): Promise<string[]> {
	try {
		...
		const result = await Bun.$`bash -c ${`ls -d ${pattern} 2>/dev/null`}`
			.quiet()
			.nothrow();
...
async function isValidStacksNodeDir(path: string): Promise<boolean> {
	...
	if (
		dirName.includes("stacks-blockchain") ||
		dirName.includes("stacks-node")
	) {
		return true;
	}
	...
	try {
		...
		const hasCompose =
			(await composeYml.exists()) || (await composeYaml.exists());
		if (!hasCompose) {
			return false;
		}
 ...
 */
async function detectNetworkFromFilesystem(
	path: string,
	...
	if (await envFile.exists()) {
		try {
			...
			if (
				content.includes("NETWORK=testnet") ||
				content.includes("STACKS_NETWORK=testnet")
			) {
				return "testnet";
			}
			if (
				content.includes("NETWORK=mainnet") ||
				content.includes("STACKS_NETWORK=mainnet")
			) {
				return "mainnet";
			}
		} catch {}
	...
	if (await activeConfig.exists()) {
		try {
			const result =
				await Bun.$`readlink ${join(path, "configurations/active")}`
					.quiet()
					.nothrow();
...
async function isComposeProjectRunning(path: string): Promise<boolean> {
	try {
		const result =
			await Bun.$`docker compose -f ${join(path, "docker-compose.yml")} ps -q`
				.quiet()
				.nothrow();
