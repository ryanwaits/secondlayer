...
export async function checkBaseDependencies(targetDir: string): Promise<void> {
	...
	if (missing.dependencies.length > 0) {
		...
		console.log(
			chalk.gray("  The generated code requires @secondlayer/stacks to work."),
		);
		...
		console.log(
			chalk.cyan(`    ${packageManager} ${installCmd} @secondlayer/stacks\n`),
		);
	}
}
...
export async function installDependencies(
	...
	if (dependencies.length > 0) {
		console.log(
			chalk.gray(`  Installing dependencies: ${dependencies.join(", ")}`),
		);
		...
		try {
			...
			console.log(
				chalk.green(`✓ Installed dependencies: ${dependencies.join(", ")}`),
			);
		} catch (error) {
	...
	if (devDependencies.length > 0) {
		...
		try {
			...
			console.log(
				chalk.green(
					`✓ Installed dev dependencies: ${devDependencies.join(", ")}`,
				),
			);
		} catch (error) {
