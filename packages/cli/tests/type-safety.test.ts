...
import { clarityTypeToTS } from "@secondlayer/scaffold";
...
describe("Type Safety - clarityTypeToTS", () => {
	describe("Primitive Types", () => {
		...
		it("handles string types", () => {
			...
			expect(clarityTypeToTS({ "string-ascii": { length: 100 } })).toBe(
				"string",
			);
			expect(clarityTypeToTS({ "string-utf8": { length: 100 } })).toBe(
				"string",
			);
		});
		...
		it("handles buffer type", () => {
			...
			expect(clarityTypeToTS("buff" as AbiType)).toBe(
				"Uint8Array | string | { type: 'ascii' | 'utf8' | 'hex'; value: string }",
			);
			// Full object format from ABI
			expect(clarityTypeToTS({ buff: { length: 32 } })).toBe(
				"Uint8Array | string | { type: 'ascii' | 'utf8' | 'hex'; value: string }",
			);
		});
	});
	...
	describe("List Types", () => {
		...
		it("handles list of tuples", () => {
			...
			expect(clarityTypeToTS(listType)).toBe("{ id: bigint; owner: string }[]");
		});
	});
				...
				response: {
					ok: {
						tuple: [
							...
							{
								name: "memo",
								type: { optional: { "string-utf8": { length: 100 } } },
							},
						],
					},
					error: "uint128",
