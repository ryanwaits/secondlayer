...
export type AssociationType = "belongs_to" | "has_many" | "has_one" | "has_many_through";
...
function findJoinTables(schema: SchemaInfo): Set<string> {
  ...
  const fksByTable = new Map<string, { fromColumn: string; toTable: string }[]>();

  for (const fk of schema.foreignKeys) {
    if (!fksByTable.has(fk.fromTable)) fksByTable.set(fk.fromTable, []);
    fksByTable.get(fk.fromTable)!.push({ fromColumn: fk.fromColumn, toTable: fk.toTable });
  }
...
export function inferAssociations(schema: SchemaInfo): AssociationMap {
  ...
  const ensure = (t: string) => { if (!map[t]) map[t] = []; };
  ...
  for (const fk of schema.foreignKeys) {
    ...
    const inverseName = isUnique ? fk.fromTable.replace(/s$/, "") : fk.fromTable;
    ensure(fk.toTable);
    map[fk.toTable].push({
