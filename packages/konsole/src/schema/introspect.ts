...
import type { ColumnInfo, ForeignKeyInfo, SchemaInfo, TableInfo } from "./types.ts";
...
export async function introspectSchema(db: Kysely<any>, pgSchema = "public"): Promise<SchemaInfo> {
  // 1. Tables
  ...
  for (const row of constraintRows) {
    ...
    if (row.constraint_type === "UNIQUE") {
      if (!uniqueMap.has(row.table_name)) uniqueMap.set(row.table_name, new Set());
