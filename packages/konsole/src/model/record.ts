...
export class Record {
  ...
  private _resolveAssociation(assoc: AssociationDef): unknown {
    ...
    switch (assoc.type) {
      case "belongs_to": {
        ...
        if (fkValue === null || fkValue === undefined) return Promise.resolve(null);
        return targetModel.find(fkValue as string | number);
      }
      case "has_many": {
        return targetModel.where({ [assoc.foreignKey]: this._attrs[this._primaryKey] });
      }
      case "has_one": {
        return targetModel.where({ [assoc.foreignKey]: this._attrs[this._primaryKey] }).first;
      }
      case "has_many_through": {
        ...
        return {
          then: async (resolve: any, reject?: any) => {
            try {
              const joinRows = await joinModel.where({ [assoc.foreignKey]: myId });
              const targetPk = targetModel._primaryKey;
              const otherFkCol = joinRows[0]
                ? Object.keys((joinRows[0] as any).attributes || joinRows[0]).find(
                    (k: string) => k !== assoc.foreignKey && k.endsWith("_id"),
                  )
                : null;
              if (!otherFkCol || joinRows.length === 0) return resolve([]);
              const ids = joinRows.map((r: any) => r.attributes?.[otherFkCol] ?? r[otherFkCol]);
  ...
  async update(attrs: Row): Promise<Record> {
    ...
    if (!pk) throw new Error(`Cannot update a record without ${this._primaryKey}`);
    await this._db
  ...
  async destroy(): Promise<boolean> {
    const pk = this._attrs[this._primaryKey];
    if (!pk) throw new Error(`Cannot destroy a record without ${this._primaryKey}`);
    await this._db
  ...
  async reload(): Promise<Record> {
    const pk = this._attrs[this._primaryKey];
    if (!pk) throw new Error(`Cannot reload a record without ${this._primaryKey}`);
