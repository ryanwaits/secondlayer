...
import type { AssociationDef, AssociationMap } from "../schema/associations.ts";
...
import type { Model, ModelRegistry, Row } from "./types.ts";
...
function createModel(
	...
	const model: Model = {
		...
		find(id: string | number) {
			return base().where({ [primaryKey]: id })
				.first as unknown as Promise<Record | null>;
		},
		findBy(conditions: Row) {
			return base().where(conditions)
				.first as unknown as Promise<Record | null>;
		},
		pluck(column: string) {
	...
	return new Proxy(model, {
		get(target, prop, receiver) {
			...
			if (!attrs.every((a) => columnSet.has(a)))
