...
import { dim, cyan, green } from "./colors.ts";
...
export function createCommands(
  ...
  return {
      ...
      for (const [tableName, table] of schema.tables) {
        ...
        if (assocs?.length) {
          for (const a of assocs) {
            console.log(dim(`    ${a.type} :${a.name} → ${a.toTable} (${a.foreignKey})`));
          }
        }
        console.log("");
      }
    ...
    async help() {
      ...
      console.log(`    ${green("Model")}.all                ${dim("all rows (lazy, chainable)")}`);
      console.log(`    ${green("Model")}.find(id)            ${dim("find by primary key")}`);
      console.log(`    ${green("Model")}.findBy({ email })   ${dim("find first matching")}`);
      console.log(`    ${green("Model")}.where({ plan })     ${dim("chainable query")}`);
      console.log(`    ${green("Model")}.first / .last       ${dim("first/last by PK")}`);
      console.log(`    ${green("Model")}.count               ${dim("count rows")}`);
      console.log(`    ${green("Model")}.pluck("col")        ${dim("array of single column")}`);
      console.log(`    ${green("Model")}.create({ ... })     ${dim("INSERT RETURNING")}`);
      console.log(`    ${green("record")}.update({ ... })    ${dim("update instance")}`);
      console.log(`    ${green("record")}.destroy()          ${dim("delete instance")}`);
      console.log(`    ${green("record")}.reload()           ${dim("re-fetch from DB")}`);
      console.log(`    ${green("record")}.association        ${dim("follow FK association")}`);
      console.log("");
      ...
      console.log(`    ${green(".counts")}                    ${dim("row counts per table")}`);
      console.log(`    ${green(".desc")} ${dim("<table>")}               ${dim("describe table columns")}`);
      console.log(`    ${green(".schema")}                    ${dim("tables + associations")}`);
      console.log(`    ${green(".relations")} ${dim("<table>")}          ${dim("associations for table")}`);
