...
export function getUnresolvedAlertForService(
	db: Database,
	service: string,
...
export function updateAlertSlackTs(
	db: Database,
	alertId: number,
	slackTs: string,
): void {
...
export function getDailySpend(db: Database): number {
	const row = db
		.query(
			"SELECT COALESCE(SUM(cost_usd), 0) as total FROM decisions WHERE created_at > datetime('now', '-1 day')",
		)
		.get() as { total: number };
	return row.total;
}
...
export function pruneOldRecords(db: Database, retentionDays = 30): void {
	...
	db.run("DELETE FROM decisions WHERE created_at < datetime('now', ?)", [
		cutoff,
	]);
	db.run("DELETE FROM snapshots WHERE created_at < datetime('now', ?)", [
		cutoff,
	]);
	db.run("DELETE FROM alerts WHERE created_at < datetime('now', ?)", [cutoff]);
	db.run(
		"DELETE FROM cooldowns WHERE last_executed_at < datetime('now', '-1 hour')",
	);
}
