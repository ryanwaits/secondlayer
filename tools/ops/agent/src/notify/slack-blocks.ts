...
export function buildAlertBlocksWithButtons(
	...
	if (opts?.alertId != null) {
		const actionBlock = buildActionButtons(
			payload.severity,
			opts.service ?? payload.service,
			opts.alertId,
		);
...
export function buildDiagnosisBlocks(
	...
	if (
		...
		!noopActions.includes(analysis.suggestedAction) &&
		!lowConfidence
	) {
		elements.push({
			type: "button",
			...
			value: JSON.stringify({
				alertId,
				service,
				action: `agent_execute:${analysis.suggestedAction}`,
