...
import { type CallbackDeps, handleSlackCallback } from "../slack-callback.ts";
...
beforeEach(() => {
	...
	globalThis.fetch = mock(async () =>
		Response.json({ ok: true, ts: "9999.9999" }),
...
describe("handleSlackCallback", () => {
	test("rejects invalid signature with 401", async () => {
		const body = makePayload("agent_dismiss", {
			alertId: 1,
			service: "indexer",
	...
	test("restart calls executor", async () => {
		...
		const body = makePayload("agent_restart", {
			alertId,
			service: "indexer",
