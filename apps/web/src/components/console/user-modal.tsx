...
export function UserModal({ open, onClose }: UserModalProps) {
	...
	return (
		<div className="user-modal-overlay" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
			<div className="user-modal">
				...
				<div className="user-modal-profile">
					...
					<div className="user-modal-info">
						<div className="user-modal-name">
							{name}
							<span className="user-modal-role">{(account?.plan ?? "member").charAt(0).toUpperCase() + (account?.plan ?? "member").slice(1)}</span>
						</div>
						<div className="user-modal-email">{email}</div>
					</div>
					<button type="button" className="user-modal-signout" onClick={() => { logout(); onClose(); }}>
						Sign out
					</button>
				</div>
				...
				<div className="user-modal-theme">
					...
					<div className="theme-switcher">
						<button type="button" className={`theme-opt${theme === "light" ? " active" : ""}`} title="Light" onClick={() => setTheme("light")}>
						...
						<button type="button" className={`theme-opt${theme === "dark" ? " active" : ""}`} title="Dark" onClick={() => setTheme("dark")}>
							<svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round">
								<path d="M13.5 8.5a5.5 5.5 0 1 1-6-6 4.5 4.5 0 0 0 6 6z" />
							</svg>
						</button>
						<button type="button" className={`theme-opt${theme === "system" ? " active" : ""}`} title="System" onClick={() => setTheme("system")}>
