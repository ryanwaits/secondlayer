...
export function DashboardContent({
	...
	const prefetchStream = useCallback(
		(id: string) => {
			qc.prefetchQuery({
				...
				queryFn: () =>
					fetch(`/api/streams/${id}`, { credentials: "same-origin" }).then(
						(r) => r.json(),
					),
				staleTime: 30_000,
			});
	...
	const prefetchSubgraph = useCallback(
		(name: string) => {
			qc.prefetchQuery({
				...
				queryFn: () =>
					fetch(`/api/subgraphs/${name}`, { credentials: "same-origin" }).then(
						(r) => r.json(),
					),
				staleTime: 30_000,
			});
	...
	const totalDeliveries = streams.reduce(
		(s, st) => s + (st.totalDeliveries ?? 0),
		0,
	);
		...
		<>
			...
			<div className="dash-stats">
				<div className="dash-stat">
					<span
						className={`dash-stat-value${streams.length === 0 ? " zero" : ""}`}
					>
				...
				<div className="dash-stat">
					<span
						className={`dash-stat-value${subgraphs.length === 0 ? " zero" : ""}`}
					>
				...
				<div className="dash-stat">
					<span
						className={`dash-stat-value${totalDeliveries === 0 ? " zero" : ""}`}
					>
				...
				<div className="dash-activity-list">
					{subgraphs.map((subgraph) => (
						...
						>
							<span
								className={`dash-activity-dot ${
									subgraph.status === "error"
										? "red"
										: subgraph.status === "syncing"
											? "yellow"
											: "green"
								}`}
							/>
							...
							<span
								className={`dash-badge ${subgraph.status === "synced" ? "active" : subgraph.status}`}
							>
								{subgraph.status}
							</span>
						</Link>
