...
export function CommandPalette() {
	...
	useEffect(() => {
		...
		const match = results.find((r) => r.action.id === response.actionId) || {
			action: actions.find((a) => a.id === response.actionId)!,
			ranges: [],
		};
		if (match?.action) {
	...
	> = {
		"/": [
			{
				...
				apiPath: "/api/streams?limit=100&offset=0",
				unwrap: (d: any) => d.streams,
			},
			{
				queryKey: queryKeys.subgraphs.all,
				apiPath: "/api/subgraphs",
				unwrap: (d: any) => d.data,
			},
		],
		"/streams": [
		...
		"/settings": [{ queryKey: ["account", "me"], apiPath: "/api/accounts/me" }],
	};
			...
			<div className="palette-overlay" onClick={closePalette}>
				...
				>
					<div className="palette-body" style={{ padding: "24px 20px" }}>
						...
						<div
							className="palette-confirm-actions"
	...
	const renderBody = () => {
		switch (mode) {
			...
			case "error":
				return (
					<div className="palette-error">{error || "Something went wrong"}</div>
				);
