...
const targetSchema = z.object({
  ...
  resourceName: z.string().optional().describe("Display name for the confirmation UI"),
});
...
export const manageResource = tool({
  ...
  inputSchema: z.object({
    action: z.enum(["pause", "resume", "disable", "enable", "replay", "delete", "revoke"])
      .describe("Action to perform on all targets"),
    resourceType: z.enum(["stream", "key"])
