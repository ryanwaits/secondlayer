...
const CREATORS: Record<string, { name: string; bio: string }> = {
	jamesbuilds: { name: "James", bio: "Building DeFi indexing tools for the Stacks ecosystem." },
	stacksdev: { name: "StacksDev", bio: "Core contributor to sBTC indexing and DeFi analytics infrastructure." },
	alexlab: { name: "Alex Lab", bio: "Building the leading DEX on Stacks with open-source analytics." },
	stackingdao: { name: "StackingDAO", bio: "Liquid stacking protocol on Stacks — making stacking accessible." },
	ryanwaits: { name: "Ryan Waits", bio: "Building Secondlayer — infrastructure for Stacks developers." },
};
				...
				<div className="overview-inner">
					...
					<div className="mp-creator-header">
						...
						<div className="mp-creator-info">
							...
							<div className="mp-creator-stats">
								<span><strong>{subgraphs.length}</strong> public subgraphs</span>
								<span><strong>{(totalQueries / 1000).toFixed(1)}k</strong> queries / 7d</span>
							</div>
						</div>
					</div>
					...
					<DetailSection title="Public Subgraphs">
						<div className="mp-grid">
							{subgraphs.map((sg) => (
								<Link key={sg.slug} href={`/marketplace/${sg.slug}`} className="mp-card">
									<div className="mp-card-header">
										<span className="mp-card-name">{sg.name}</span>
										<span className={`mp-card-status ${sg.status}`}>{sg.status}</span>
									</div>
