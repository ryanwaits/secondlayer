...
import { Banner, Insight } from "@/components/console/intelligence";
import { useBreadcrumbOverrides } from "@/lib/breadcrumb";
import { formatDate, formatNum, relativeTime } from "@/lib/format";
import {
	detectDeliveryGap,
	detectFailurePattern,
} from "@/lib/intelligence/streams";
import {
	useDeleteStream,
	useDeliveries,
	useDisableStream,
...
export function StreamDetailClient({
	...
	const { data: stream = initialStream } = useStream(
		initialStream.id,
		initialStream,
	);
	...
	const [deleting, setDeleting] = useState<"idle" | "confirm" | "deleting">(
		"idle",
	);
	...
	return (
		<>
			...
			<pre className="dash-code-block">{highlightJson(stream.filters)}</pre>
			...
			<div style={{ display: "flex", gap: 8 }}>
				...
				{deleting === "confirm" ? (
					<div className="dash-callout-warn">
						Are you sure? This will permanently delete this stream and all its
						delivery history.
...
function StreamIntelligence({
	...
	return (
		...
		>
			{gap && (
				<Banner variant="info">
					This stream has a gap of{" "}
					<strong>{gap.missedBlocks.toLocaleString()} blocks</strong> (#
			...
			{failure && (
				<Insight variant="danger" id={`failure-${stream.id}`}>
					...
					<strong>{failure.sinceDuration}</strong>. Your endpoint may be down or
