...
export function ReplayClient({ stream }: { stream: Stream }) {
  ...
  const blockCount =
    fromBlock && toBlock ? Math.max(0, Number(toBlock) - Number(fromBlock) + 1) : 0;
  ...
  return (
    <>
      ...
      <div style={{ display: "flex", gap: 12, alignItems: "flex-end", marginTop: 4 }}>
        <div style={{ flex: 1 }}>
          <label style={{ display: "block", fontSize: 12, fontWeight: 500, color: "var(--text-muted)", marginBottom: 6 }}>Start block</label>
          <input
        ...
        <div style={{ flex: 1 }}>
          <label style={{ display: "block", fontSize: 12, fontWeight: 500, color: "var(--text-muted)", marginBottom: 6 }}>End block</label>
          <input
            className="dash-input"
            type="text"
            inputMode="numeric"
