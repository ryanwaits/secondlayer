...
export function EndpointClient({ stream }: { stream: Stream }) {
	...
	return (
		<>
			<div className="dash-page-header">
				<h1 className="dash-page-title">Endpoint</h1>
			</div>
							...
							>
								...
								>
									{secretRevealed
										...
										: `${(stream.signingSecret ?? "slsec_").slice(0, 6)}${"•".repeat(12)}`}
								</code>
								{secretCopied && (
			...
			<div className="dash-index-group">
				...
				/>
				<OptionRow label="includeRawTx" value={stream.options.includeRawTx} />
				<OptionRow label="timeoutMs" value={stream.options.timeoutMs} />
				<OptionRow label="maxRetries" value={stream.options.maxRetries} />
			</div>
