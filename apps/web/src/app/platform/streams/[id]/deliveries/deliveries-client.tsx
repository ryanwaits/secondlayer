...
function DeliveryPayload({
	streamId,
	deliveryId,
}: { streamId: string; deliveryId: string }) {
	...
	const prefetchPayload = useCallback(
		(deliveryId: string) => {
			qc.prefetchQuery({
				queryKey: ["delivery", deliveryId],
				queryFn: () =>
					fetch(`/api/streams/${stream.id}/deliveries/${deliveryId}`, {
						credentials: "same-origin",
					}).then((r) => r.json()),
				staleTime: 60_000,
			});
		...
		<>
			{isError ? (
				<div className="dash-empty">
					Failed to load deliveries. Try refreshing.
				</div>
				...
				<>
								...
								return (
									<tbody key={d.id}>
										...
										>
											...
											<td>
												<span
													className={`dash-badge ${isOk ? "active" : "failed"}`}
												>
													{d.statusCode}
												</span>
											</td>
					...
					>
						<span>
							...
							{Math.min((page + 1) * PAGE_SIZE, stream.totalDeliveries)} of{" "}
							{stream.totalDeliveries.toLocaleString()}
						</span>
