							...
							{colEntries.map(([colName, col]) => (
								<div key={colName} className="col-row">
									...
									<span className="col-badges">
										...
										{col.searchable && (
											<span className="badge-searchable">searchable</span>
										)}
										{col.nullable && (
											<span className="badge-system">nullable</span>
										)}
										{table.uniqueKeys?.some(
											(uk) => uk.length === 1 && uk[0] === colName,
							...
							<>
								...
								<div className="index-pills">
									{indexPills.map((pill, i) => (
										<span key={i} className="index-pill">
											{pill}
										</span>
									))}
								</div>
