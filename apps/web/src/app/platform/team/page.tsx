...
import { useProjects, useTeamMembers, useInviteTeamMember } from "@/lib/queries/projects";
		...
		<>
			...
			<div className="settings-scroll">
				<div className="settings-inner">
					...
					<p className="settings-desc">Manage who has access to this project and their roles.</p>
					...
					{showInvite && (
						<div style={{ padding: "14px 16px", border: "1px solid var(--border)", borderRadius: 8, marginBottom: 16 }}>
							<form onSubmit={handleInvite} style={{ display: "flex", alignItems: "flex-end", gap: 10 }}>
								<label style={{ flex: 1 }}>
									<span className="settings-label">Email</span>
									<input className="settings-input" type="email" placeholder="team@example.com" value={email} onChange={(e) => setEmail(e.target.value)} autoFocus />
								</label>
								<label>
									<span className="settings-label">Role</span>
									<select className="settings-input" value={role} onChange={(e) => setRole(e.target.value)} style={{ width: 120 }}>
										<option value="member">Member</option>
										<option value="admin">Admin</option>
									</select>
								</label>
								<button type="submit" className="settings-btn primary" disabled={inviteMember.isPending}>
									{inviteMember.isPending ? "..." : "Invite"}
								</button>
								<button type="button" className="settings-btn ghost" onClick={() => { setShowInvite(false); setEmail(""); }}>
									Cancel
								</button>
							</form>
						</div>
					...
					<div className="settings-section">
						{members.length === 0 ? (
							...
							members.map((m) => {
								const initial = (m.displayName || m.email || "U")[0].toUpperCase();
								return (
									<div key={m.id} className="member-row">
										...
										<div className={`member-role${m.role === "owner" ? " owner" : ""}`}>
											{m.role}
										</div>
									</div>
								);
							...
							invitations.map((inv) => (
								<div key={inv.id} className="member-row">
									...
									<div className="member-info">
										...
										<div className="member-email">Expires {new Date(inv.expiresAt).toLocaleDateString()}</div>
									</div>
