...
Reads hit your own Postgres: no meter, no rate limit. `from_height=0` reaches whatever history your instance holds; restoring more from the signed archive is metered ([archive credits](/docs/authentication#archive-credits)). Loopback needs no token; hosted `api.secondlayer.tools` is keyed ([Authentication](/docs/authentication)).
...
```bash
secondlayer codegen index --target kysely   # also: prisma, drizzle, json-schema
```
...
Where to run it: [Deploy](/docs/deploy). Running balances: invert them in a sink's `onRollback` ([Sinks](/docs/sinks#rollback-is-height-stamped)).
...
Boot-contract feeds use the same verbs and envelope. They are Index on contracts everyone shares, not extra products. Your contract stays in `consume()` or a [subgraph](/docs/subgraphs).

| Endpoint | Returns | Filters beyond the common set |
| --- | --- | --- |
| `/v1/index/stacking` | Decoded PoX-4 stacking actions (historical: era ended at Epoch 4.0; live: `/v1/index/pox5/events`) | `function_name`, `stacker`, `caller` |
| `/v1/index/pox5/events` | Decoded PoX-5 print events, all 19 topics ([PoX-5 events](/docs/pox5-events)) | `topic`, `staker`, `signer`, `signer_manager`, `bond_index`, `reward_cycle` |
| `/v1/index/sbtc/*` | Decoded sBTC peg lifecycle ([sBTC settlement](/docs/sbtc-settlement)) | deposits, withdrawals, events, summary |
...
`tip.block_height` is the committed height for the event type you just read, not a shared global number. Each decoded type (`ft_transfer`, `stx_transfer`, `print`, and so on) runs its own decoder, and they don't all sit at the same height. A `stx_transfer` read and an `ft_transfer` read against the same instance can report different tips at the same moment; both are correct for what they cover. `/v1/index/blocks` and `/v1/index/transactions` stay on the ingest tip (`source_block_height`) since raw blocks land with the chain, not with decode. If you cache a tip across event types, re-read it per response instead of reusing one number.

Built on [Streams](/docs/streams): the decoder on your instance reads the same raw firehose you can.

### Long-poll instead of tight-loop polling

`/v1/index/events` and `/v1/index/blocks` take `wait` (seconds, max 25). With `wait`, an empty page — nothing past your `cursor`/`from_height` yet — holds the connection open instead of answering immediately, and returns the moment new data commits or `wait` elapses, whichever comes first. A page that already has rows answers right away regardless of `wait`.

```
GET /v1/index/events?event_type=ft_transfer&cursor=<last>&wait=25
```

This replaces a `poll every N seconds` loop with `ask, then ask again` — same cursor logic, fewer empty round trips. A self-hosted instance from before `wait` shipped rejects the param (`400`); fall back to a plain poll loop on that response instead of retrying with `wait` forever.

## Get told when it changes

A `consume()` loop pulls. To be pushed instead, into a queue or a serverless function, a webhook POSTs matching chain events to your URL as they land:
...
Deliveries are signed, retried, and land in a dead-letter queue you can inspect and requeue. Full trigger grammar and the delivery envelope: [Webhooks](/docs/webhooks).
...
MCP exposes the primitive families as tools: `index_events`, `index_transactions`, `index_mempool`, `index_blocks`, and the rest. Boot-contract tools (`index_stacking`, pox-5, sBTC) are the same Index, on shared contracts.

export const metadata = {
	title: "Index — Secondlayer Docs",
	description:
		"Decoded Stacks data, kept indexed: sweep it into your own tables with a checkpointed consumer, cursors, reorg rewind, and codegen.",
};
