...
Need the submitting transaction? Pass `tx_context=true`. Every event then carries its tx joined in — `tx_sender`, `tx_type`, `tx_status`, `tx_contract_id`, `tx_function_name` — so [building your own index](#build-your-index-on-it) skips a `/v1/index/transactions` call per event, turning an N+1 transaction walk into one pass.
...
Decoded print events from the `pox-5` boot contract (SIP-045 Bitcoin Staking), which replaces PoX-4 at the epoch 4.0 hard fork — all 19 topics, filterable by `topic`, `staker`, `signer`, `signer_manager`, `bond_index`, or `reward_cycle`. Hot fields are promoted to typed columns, and every row also carries `data`, the full decoded tuple, so nested shapes with no flat column (`btc-lockup`, `bond-rewards`, `bond-periods`) are never lost.
...
For contract-specific indexes, [`/print-schema`](#print-schemas) tells you the real shape of a contract's `print` payloads before you design tables; then walk `event_type=print&contract_id=…` into them — add `tx_context=true` to capture the submitting sender in one pass (prints carry none) instead of fetching each transaction. Transaction [inclusion proofs](#proofs) let you verify anything you mirrored without trusting us.
