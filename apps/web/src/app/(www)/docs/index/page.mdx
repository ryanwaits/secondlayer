...
Reads need no key, and a minted free-tier key reads Index too — at the free-tier rate limit, never slower than anonymous. Free reads cover the recent 24-hour window; reaching older history is [pay-as-you-go](/docs/authentication#pay-as-you-go-credits) (prepaid credits) or a paid plan.
...
Free and anonymous reads are limited to that recent 24-hour window: a `from_height`/`cursor` below it returns `402 UPGRADE_REQUIRED`. Top up [pay-as-you-go credits](/docs/authentication#pay-as-you-go-credits) (or start a plan) to read older history.
...
Pending, unconfirmed transactions with the same decoded enrichment as `/transactions`, but pre-chain: no `block_height` or `result`, plus a `received_at`. Filter by `sender`, `type`, or `contract_id`.

<Callout type="note">

Rows leave the mempool on confirmation or drop, so the single form `404`s once a transaction mines. This surface is never cached.
...
To reconcile: roll back any rows whose cursor falls inside `orphaned_range`, then re-fetch from `new_canonical_tip`. The SDK exports the `IndexReorg` type from `@secondlayer/sdk` for typing these elements, and the [ingest loop below](#build-your-index-on-it) shows the reconcile in code.
