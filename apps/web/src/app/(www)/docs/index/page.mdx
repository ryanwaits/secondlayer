...
Reads on your instance are free and unlimited. `from_height=0` reaches whatever history your instance holds; restoring more from the signed archive is metered ([archive credits](/docs/authentication#archive-credits)).
...
Backs `secondlayer subgraphs create <name> --from-contract <contract_id>` — see [Subgraphs](/docs/subgraphs#typed-print-payloads).
...
A `consume()` loop pulls. If you'd rather be pushed — into a queue, a serverless function, another service — a subscription POSTs matching chain events to your URL as they land, with no polling loop to run:
