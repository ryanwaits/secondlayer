...
[Index](/docs/index) is built on this exact surface (our decoder is itself a Streams consumer). Page forward with a cursor, replay from any point, and with a committed checkpoint, within your tier's retention, never miss an event. Stream live over SSE in-process (below), or fan out to your backend with [Subscriptions](/docs/subscriptions).
