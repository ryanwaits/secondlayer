...
| --- | --- |
| `/extended` | not served |
| watches | throw `SimnetUnsupportedError` |
| fees | `NoEstimateAvailable`; wallet actions use `'min'` |
...
- **Typed contracts**: `getContract()` binds an ABI to a client for typed reads (`read.*`), broadcasts (`call.*`), unsigned wallet-signs-later transactions (`buildCall.*`), and map lookups (`maps.*`); generate branded ABIs with [`secondlayer codegen contracts`](/docs/cli#generate-typed-contract-clients) for named-alias hovers.
