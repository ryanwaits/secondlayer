...
Epoch 4.0 activates at Bitcoin block **960,230** (~July 29, 2026). The module is pinned against the final `pox-5` contract in **stacks-core 4.0.0** — interface-checked in Clarinet simnet, script and grant-hash ports byte-compared against the contract's own read-onlys in CI. Everything off-chain works **today**; contract calls go live when the chain crosses the activation height, and your code finds that out by asking the chain, not a constant.
...
`isActive()` and `getActivation()` read `/v2/pox` `contract_versions` — the node reports where `pox-5` activates on *its* network, so the same code is correct on mainnet, testnet, and devnet.
...
Individual reads are exposed too — an optional read returns `null` when the record doesn't exist: `getStakerInfo`, `getBondMembership`, `getProtocolBond`, `getBondAllowance`, `getTotalSbtcStakedForBond`, `getStakerCustodiedSbtc`, `hasAnnouncedL1EarlyExit`, `getBondL1UnlockHeight`, `getSignerInfo`, `verifySignerKeyGrant`, `getCurrentRewardCycle`, `getFirstRewardCycle`.
...
Byte-for-byte TypeScript ports of the contract's script constructors — the contract validates your L1 output against *exactly* these bytes.
