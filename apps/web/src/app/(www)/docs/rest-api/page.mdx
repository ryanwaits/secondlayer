...
Every `/v1` surface answers with the same envelope, cursor, and filter grammar, so one client shape reads Index, Streams, and subgraphs. Served at `http://127.0.0.1:3800` (`SECONDLAYER_API_URL` overrides); tokens: [Authentication](/docs/authentication).
...
| Route | Returns |
| --- | --- |
| `GET /v1/subgraphs` | Your instance's subgraphs |
...
Reads are open on loopback with wildcard CORS; published past loopback they require the `INSTANCE_TOKEN` bearer. Same rule on `/v1/index` and `/v1/streams` — there is no per-subgraph visibility flag, so who can reach the instance is what your publish spec and reverse proxy decide. Hosted `/v1` is keyed. See [Authentication](/docs/authentication).
...
Typed as `sl.subgraphs.typed(def).<table>.aggregate(spec)`, or untyped as `sl.subgraphs.queryTableAggregate(name, table, params)`: signatures in the [SDK reference](/docs/sdk-reference).
...
`POST /v1/batch` runs up to 10 `/v1` reads in one round trip, for agents and dashboards that pay per request in latency.

```bash
curl -X POST http://127.0.0.1:3800/v1/batch -H "content-type: application/json" -d '{
  "requests": [
    { "path": "/v1/index/pox5/events", "params": { "topic": "stake", "limit": 5 } },
    { "path": "/v1/streams/tip" }
  ]
}'
```

Results come back in order as `{ "results": [{ "path", "status", "body" }] }`. Each item runs through the same auth and limits as a direct call, with your `Authorization` header forwarded. Only paths under `/v1/index/`, `/v1/subgraphs`, `/v1/streams/` and `/v1/contracts` are allowed; anything else is a per-item `400`.
