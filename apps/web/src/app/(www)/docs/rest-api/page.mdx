...
The shared envelope, cursor, and filter grammar across index, streams, and subgraph tables. Your instance serves it at `http://127.0.0.1:3800` (`SL_API_URL` overrides); reads are free and unlimited, tokens: [Authentication](/docs/authentication).
...
Reads are open on loopback with wildcard CORS; a non-loopback bind requires the `INSTANCE_TOKEN` bearer.
