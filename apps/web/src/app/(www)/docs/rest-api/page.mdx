...
Every `/v1` surface answers with the same envelope, cursor, and filter grammar, so one client shape reads Index, Streams, and subgraphs. Served at `http://127.0.0.1:3800` (`SL_API_URL` overrides); tokens: [Authentication](/docs/authentication).
