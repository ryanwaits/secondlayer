...
The other formats — `raw`, `cloudevents`, `trigger`, `cloudflare`, `inngest` — don't carry this HMAC. They're authenticated by the universal ed25519 signature in [Universal webhook signature](#universal-webhook-signature) below, which rides on **every** delivery regardless of format.
