...
Via REST, `POST /api/subscriptions` accepts the same `triggers` array (1–50). Chain subscriptions are created over the SDK, REST, or MCP; the CLI's `create` is subgraph-only.
