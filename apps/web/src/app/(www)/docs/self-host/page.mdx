...
safe default for either. `--skip-bootstrap` skips the archive: the index then
holds only what your node sends from now on, full history only if the node
itself syncs from genesis. `--skip-verify` skips the post-bootstrap check.
