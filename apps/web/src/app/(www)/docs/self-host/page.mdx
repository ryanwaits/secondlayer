...
You skip the platform-only layer (managed SLA, billing/credits, multi-tenant key management, public Explore directory). Support is community/best-effort. For managed hosting with uptime SLAs and priority support, see [pricing](/pricing).

---

## Prerequisites

**Docker + Docker Compose** (v2) is the only hard requirement for the app services. You also need a Stacks node for the event feed: run one yourself, or point the indexer at an existing node's event observer.
...
OSS mode sets `INSTANCE_MODE=oss`, which disables the platform-only layer (magic-link auth, API keys, billing/credits, x402 pay-per-call, multi-tenant). Single-tenant; the operator owns access control.
...
During genesis-to-tip sync, disable the tip follower so it doesn't interfere with IBD:
...
OSS runs on a **single Postgres instance**. The source/target DB split in `docker/SCHEMA_SPLIT.md` is a hosted-scale concern for high-throughput multi-tenant deployments; you don't need it for self-hosting.
...
OSS disables the platform-only layer described in the [intro](#self-host):

- **Auth/keys**: use the `API_KEY` env for a single shared bearer token; layer your own reverse proxy for finer control.
...
- **Explore**: [secondlayer.tools/subgraphs/explore](https://secondlayer.tools/subgraphs/explore) is managed-only.
