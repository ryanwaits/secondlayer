...
| To build from the raw inputs | **Streams** | The signed raw event firehose with parquet dumps, replay, and a checkpointed consumer with automatic reorg rewind. For data engineers building an indexer from zero — it's what Index itself runs on. |
