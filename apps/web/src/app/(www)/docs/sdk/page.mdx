...
- **`fromHeight: 0`** backfills from genesis on your instance. Official-archive restore is [metered](/docs/authentication#archive-credits).
- **`finalizedOnly`** holds delivery to rows at or below `tip.finalized_height`.

A [sink](/docs/sinks) removes the checkpoint, transaction, and rollback code
above entirely — it owns all three, and your handler only inserts rows:
...
See [Sinks](/docs/sinks), [Index, Build your index on it](/docs/index#build-your-index-on-it), and the runnable [`sales-index` example](https://github.com/ryanwaits/secondlayer/tree/main/examples/sales-index).
...
- `sl.subgraphs.rows(name, table, opts)` returns `{ rows, next_cursor, tip }` from the open `/v1` read. Keyless on loopback; pass `INSTANCE_TOKEN` on a non-loopback bind.
