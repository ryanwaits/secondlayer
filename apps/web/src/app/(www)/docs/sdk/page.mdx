...
On Index, `consume` works over decoded events and contract calls. Reorgs rewind the cursor to the fork point automatically (`onReorg` fires so you can roll back committed rows); `fromHeight: 0` backfills from genesis; `finalizedOnly` holds delivery to rows at or below `tip.finalized_height`.
