...
`subscribe({ fromCursor?, types?, notTypes?, contractId?, sender?, recipient?, assetIdentifier?, signal?, onEvent, onError? }): () => void`. When the client was created with `verify`, each frame's ed25519 signature is verified before `onEvent` runs.
...
```ts
const unsubscribe = sl.subgraphs.sbtcFlows.transfers.subscribe(
  (row) => {
    // handle each new row
  },
  { since: 8054704 },
