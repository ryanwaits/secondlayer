...
`status`, `block`, and `tx` are live. Events, address, tokens, and balances are not. Node RPC (`/v2`) stays on the node. Missing Hiro fields (`miner_txid`, `execution_cost_*`, `post_conditions`) are omitted, never zeroed.
...
| Query | Default | Notes |
| --- | --- | --- |
| `limit` | 20 | Max 30. Over → 400 |
| `offset` | 0 | |
| `from_height`, `to_height` | | `tx` list only |

Lists use `{ limit, offset, total, results }`, not the `/v1` cursor envelope. `cursor` and `from_cursor` return 400.
