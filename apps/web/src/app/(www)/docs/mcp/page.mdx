...
`SL_API_KEY` is optional; leave it out for anonymous reads (see [Authentication](#authentication)).
...
An agent holding an account-level key can mint scoped read keys with `account_create_key`. Keys and signing secrets are returned **once**: capture and forward them to the user.
...
Thirty-three tools across seven domains. Reads are cursor-paginated; writes that kick off work (reindex, backfill, replay, deploy) return an `operationId` to poll.
