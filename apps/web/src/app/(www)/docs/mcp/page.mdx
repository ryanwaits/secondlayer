...
`INSTANCE_TOKEN` is optional for reads on loopback; anything published past loopback requires it, and the write tools require it always (see [Authentication](#authentication)).
...
One rule across all of them: reads are open while the API is published on loopback and take the token past it; writes take the token whenever the instance has one, loopback included. Set `INSTANCE_TOKEN` in the server's env and every tool works either way — a token sent on a read that didn't need it is ignored, never rejected. Hosted archive and credits tools use `SECONDLAYER_API_KEY`; see [Authentication](/docs/authentication).
...
export const metadata = {
	...
	description:
		"The Secondlayer MCP server: stdio and HTTP transports, resources, and tools for Index, Subgraphs, Streams, Subscriptions, and contracts.",
};
