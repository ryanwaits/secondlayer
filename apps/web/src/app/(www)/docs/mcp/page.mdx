...
`INSTANCE_TOKEN` is optional for reads on loopback; anything published past loopback requires it, and the write tools require it always. `SECONDLAYER_API_URL` is the lead instance URL. Hosted archive/credits tools also need `SECONDLAYER_API_KEY`. See [Authentication](/docs/authentication).
