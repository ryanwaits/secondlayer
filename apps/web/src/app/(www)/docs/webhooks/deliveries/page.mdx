...
<Callout type="warning" title="Delivery semantics">

Delivery is **at-least-once** and **unordered**: dedupe on `(tx_id, event_index, block_hash)` and don't assume arrival order matches chain order. On a reorg, `chain.reorg.rollback` covers everything that *may* have reached your endpoint, meaning any row the emitter claimed or attempted, not only one the server confirmed as delivered. A delivery whose response never made it back still counts as "may have arrived," so undo every entry the rollback lists.

</Callout>

## Verify the signature

On the default `standard-webhooks` format, every delivery carries a per-webhook HMAC. Pass the **raw body first**.
...
A chain-webhook delivery and a [Streams](/docs/streams) event are structurally unrelated, so don't parse one as the other. Streams events are `{ event_type, payload, cursor, block_height, … }`; a chain-webhook delivery is `{ type, timestamp, data: { trigger, event } }`.
...
Per-trigger shapes for `data.event`: [Event shapes](/docs/webhooks/event-shapes).
