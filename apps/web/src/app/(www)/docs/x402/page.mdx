...
> **Experimental.** The pay-per-call rail is a beta; surfaces, prices, and headers may change. Plans on the [pricing page](/pricing) are the stable way to pay. Built for agents: pay Index and Streams reads (`/v1/index`, `/v1/streams`) with **x402**, the HTTP 402 payment standard settled on Stacks, and a paid `POST /v1/subgraphs` deploys an indexer owned by your wallet. No card, no signup, no gas: transfers are sponsored, you only hold the token (sBTC, STX, or USDCx).

## How it works

### 1. Call without a key

Call without a key while the rail is on, and the API answers `402 Payment Required` with exact quotes per accepted token.
...
Whether a request gets a free `200` or a `402` quote is decided per-request, in this order:

1. **Holding an API key bypasses x402.** A request with a valid `sk-sl_` key is an account-backed caller; it never sees a 402, and usage bills against your [plan](/pricing) via Stripe. Keyless uses the pay-per-call rail; keyed uses your plan.
2. **The rail has to be on.** Pay-per-call is gated on a funded sponsor wallet (`X402_SPONSOR_KEY`). While it's off, keyless Index reads stay free and Streams stays key-mandatory. Check `GET /.well-known/x402` (`enabled: true|false`) for the live state.
3. **Index keeps a free anon allowance.** With the rail on, the first **1,000 keyless reads per IP per day** on `/v1/index` still return `200`. The 402 only starts once that daily budget is spent.
...
The same endpoint answers three ways for the same agent:
...
Standard x402 v2 wire works with any x402 client; sponsored means we pay the STX gas. Discover the rail at `GET /.well-known/x402` and the `x-x402` block in [the OpenAPI spec](https://api.secondlayer.tools/v1/openapi.json).
