...
| Job | Command |
| --- | --- |
| Check identity columns | `sl verify --against <latest.json>` |
| Also check payload columns | `sl verify --against <latest.json> --semantic` |
...
| `--semantic` | Recompute per-partition semantic digests locally and compare. Catches divergence in `raw_tx`, event `data`, and `function_args` that identity-only digests skip. |
...
Default pass: identity columns (~90 s for mainnet). `--semantic` after a rebuild or against a foreign runtime.
