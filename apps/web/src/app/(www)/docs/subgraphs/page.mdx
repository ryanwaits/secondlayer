...
Subgraphs run on your instance's bundled Postgres by default. `--database-url` lands rows in a **separate** Postgres you point at instead, then codegen a typed ORM schema:

```bash
secondlayer subgraphs deploy ./subgraph.config.ts --database-url "$DATABASE_URL"
...
A breaking change (removed table/column, changed column type, forced reindex) auto-reindexes subgraphs on the bundled instance database by dropping and rebuilding the schema. On a `--database-url` database that drop would destroy data Secondlayer doesn't own, so the deploy is **refused** (`422`) — nothing on your database is touched.
