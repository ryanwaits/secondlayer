...
Index on the sBTC peg contracts, plus Bitcoin confirmation of sweeps. Same `consume()` / `walk()` loop as any other decoded feed. Not an sBTC product.
...
curl "http://127.0.0.1:3800/v1/index/sbtc/withdrawals?settlement_confirmed=false"
```
...
See [Webhooks](/docs/webhooks) for delivery formats, signature verification, and the full trigger list.
