# CLI

The `sl` command-line tool — authenticate once, manage subgraphs and subscriptions from your terminal.

## Install

```bash
bun add -g @secondlayer/cli
```

## Common commands

```bash
sl login                 # authenticate (opens the browser)
sl subgraphs deploy      # deploy a subgraph from a config
                         #   --visibility public|private (managed → public, BYO → private)
sl subgraphs publish     # claim the global public name (409 PUBLIC_NAME_TAKEN if claimed)
sl subgraphs unpublish   # make a subgraph private again
sl subgraphs ls          # list your deployments
sl subscriptions create  # wire up a webhook subscription
```

Every tool shares one session — sign in once and the SDK, CLI, and MCP server all inherit it.

## Orientation

```bash
sl context               # headless orientation snapshot
```

`sl context` prints a snapshot to orient before acting — your account, the live Streams and Index tips, your subgraphs, your subscriptions, and any in-flight reindex operations. It's the CLI counterpart to the MCP `secondlayer://context` resource agents read before they reach for a tool.

`sl subscriptions create` wires a webhook to a subgraph table. [Direct chain subscriptions](/docs/subscriptions#direct-chain-subscriptions-no-subgraph) — webhooks on raw chain events with no subgraph — are created over the SDK, REST, or MCP instead (they take a `triggers` array rather than a table).

## ORM codegen

Generate a typed ORM schema for a subgraph's tables — pair it with `--database-url` to query your own Postgres with Prisma, Drizzle, or Kysely (joins, relations, transactions):

```bash
sl subgraphs codegen ./subgraph.config.ts --target prisma -o prisma/schema.prisma
sl subgraphs codegen ./subgraph.config.ts --target drizzle -o db/schema.ts
sl subgraphs codegen ./subgraph.config.ts --target kysely -o db/types.ts
```

See [Subgraphs](/docs/subgraphs#use-your-own-database--orm) for wiring each ORM to your BYO database.

## Local devnet

Run the full pipeline against a local [Clarinet](https://docs.hiro.so/stacks/clarinet) devnet — write Clarity, `clarinet devnet start`, and index it locally with subgraphs, datasets, and subscriptions.

```bash
cd my-clarinet-project
sl devnet connect        # patches Devnet.toml + starts the Secondlayer stack
clarinet devnet start    # your normal command — events now stream to the indexer

SL_API_URL=http://localhost:3800 SL_API_KEY=dummy \
  sl subgraphs deploy ./subgraph.ts
```

`connect` pulls the published OSS images (Docker required) — no repo checkout needed. Stop with `sl devnet down` (add `--purge` to wipe the local index when restarting your devnet from scratch).

To populate rows you need a real contract-call transaction (`clarinet console` is simnet-only and won't broadcast on-chain). Fire one with `@stacks/transactions`:

```ts
import {
  broadcastTransaction,
  getAddressFromPrivateKey,
  makeContractCall,
} from "@stacks/transactions";

const key =
  "753b7cc01a1a2e86221266a154af739463fce51219d97e4f856cd7200c3bd2a601"; // devnet deployer
const sender = getAddressFromPrivateKey(key, "testnet");
const { nonce } = await fetch(
  `http://localhost:3999/v2/accounts/${sender}?proof=0`,
).then((r) => r.json());

const tx = await makeContractCall({
  contractAddress: "ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM",
  contractName: "counter",
  functionName: "increment",
  functionArgs: [],
  senderKey: key,
  network: "devnet",
  fee: 3000n,
  nonce: BigInt(nonce),
});
console.log(await broadcastTransaction({ transaction: tx, network: "devnet" }));
```

The row appears at `/v1/subgraphs/<name>/<table>` within ~5s.

export const metadata = {
	title: "CLI — Secondlayer Docs",
	description: "Manage subgraphs and subscriptions from your terminal with sl.",
};
