...
[Verified archive](/docs/archive) owns `secondlayer verify` / `secondlayer repair`. [Self-host](/docs/self-host) is the compose loop.
...
- `secondlayer subgraphs publish` / `unpublish` are hidden. Visibility is on [Subgraphs](/docs/subgraphs#visibility--publishing) and deploy `--visibility`.
