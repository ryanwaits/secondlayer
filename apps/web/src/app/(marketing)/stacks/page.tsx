  ...
  account: privateKeyToAccount("..."),
				...
				<div className="prose">
					<p>
						...
						Clients are extensible with <code>.extend()</code> for BNS, PoX, and
						StackingDAO modules.
					</p>
					<p>
				...
				<CodeBlock
					code={`import { createPublicClient, http, webSocket, fallback } from "@secondlayer/stacks"
import { mainnet } from "@secondlayer/stacks/chains"
...
const fee = await publicClient.estimateFee({ ... })`}
				...
				<CodeBlock
					code={`import { readContract, callContract } from "@secondlayer/stacks/actions"
...
const results = await multicall(publicClient, [
  ...
  { contract: "SP...::token-b", method: "getBalance", args: { owner: "SP..." } },
				...
				<div className="prose">
					<p>STX transfers with post-conditions for safety.</p>
				</div>
...
const txId = await transferStx(wallet, {
  ...
  postConditions: [
    Pc.principal("SP1234...").willSendLte(1_000_000n).ustx(),
				...
				<CodeBlock
					code={`import { watchBlocks, watchMempool, watchTransaction,
				...
				<div className="props-section">
					...
					<div className="prop-row">
						...
						<span className="prop-type">
							readContract, callContract, transferStx, deployContract,
							multicall, simulateCall, getContract, getMapEntry, getContractAbi,
							getBalance, getBlockHeight, getBlock, getNonce, estimateFee,
							getAccountInfo
						</span>
					</div>
					<div className="prop-row">
						...
						<span className="prop-type">
							buildTokenTransfer, buildContractCall, buildContractDeploy,
							signTransaction, signMessage, sponsorTransaction
						</span>
					</div>
					<div className="prop-row">
						<span className="prop-name">subscriptions</span>
						<span className="prop-type">
							watchBlocks, watchMempool, watchTransaction, watchAddress,
							watchAddressBalance, watchNftEvent
						</span>
					</div>
					<div className="prop-row">
						<span className="prop-name">postconditions</span>
						<span className="prop-type">
							Pc fluent builder for post-conditions
						</span>
					</div>
					<div className="prop-row">
						<span className="prop-name">clarity</span>
						<span className="prop-type">
							Cl constructors, serialize/deserialize, ABI types
						</span>
					</div>
					<div className="prop-row">
					...
					<div className="prop-row">
						...
						<span className="prop-type">
							formatStx, parseStx, address validation, encoding
						</span>
					</div>
				</div>
			</main>
