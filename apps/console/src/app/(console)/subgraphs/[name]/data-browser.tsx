...
function fetchPage(subgraphName: string, table: string, page: number) {
	return async () => {
		const res = await consoleFetch(
			`/api/subgraphs/${encodeURIComponent(subgraphName)}/${encodeURIComponent(table)}?_limit=${LIMIT}&_offset=${page * LIMIT}&_sort=_block_height&_order=desc`,
		);
...
export function SubgraphDataBrowser({
	...
	const { data, isLoading, isFetching, isPlaceholderData } = useQuery({
		queryKey,
		queryFn: fetchPage(subgraphName, activeTable, page),
		staleTime: 30_000,
	...
	useEffect(() => {
		...
		if ((page + 1) * LIMIT < total) {
			queryClient.prefetchQuery({
				...
				queryFn: fetchPage(subgraphName, activeTable, page + 1),
				staleTime: 30_000,
			});
		}
	}, [data, page, subgraphName, activeTable, queryClient]);
