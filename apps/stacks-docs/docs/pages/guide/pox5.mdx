...
Reading bond prints from your instance is on [PoX-5 events](https://secondlayer.tools/docs/pox5-events). This page is the wallet and proof side, from `@secondlayer/stacks`.

:::info[Wallet actions gate on chain-reported activation.]
...
Proof fields come from `buildTxProof` in [`@secondlayer/stacks/bitcoin`](/guide/bitcoin-spv); the contract verifies the lockup tx's Bitcoin inclusion on-chain.
...
:::warning[Prepare phase rejects exits]
...
Semantics: [SIP-045 staking post-conditions](/guide/transactions#sip-045-staking-post-conditions). See also the [Stacks SDK](/guide/transactions) and [Bitcoin SPV](/guide/bitcoin-spv).
