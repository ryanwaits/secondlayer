...
:::info[On-chain at Epoch 4.0, Bitcoin block 960,230]
...
:::note[`contract` is optional on mainnet]
...
Drive it with [`getContract` + `simnet()`](/guide/transactions#test-against-clarinet-simnet).
...
:::note[SPV trust-minimizes verification, not custody]
...
See also [Verification](https://secondlayer.tools/docs/verification) for inclusion proofs on the *Stacks* side, and the [SDK](https://secondlayer.tools/docs/sdk).
