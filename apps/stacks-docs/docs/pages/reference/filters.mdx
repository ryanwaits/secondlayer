...
One chain-event filter vocabulary for Index, Streams, Webhooks, and Subgraphs.

Each product used to spell the same event differently (three incompatible `minAmount` types among them). `@secondlayer/stacks/filters` is the single spelling; every surface's wire shape is a projection of it. A field a surface cannot express throws at projection time instead of silently widening the match.
...
The same filter feeds all four products.

```typescript
sl.index.events.list(usdc.toIndexParams({ limit: 100 }));               // pull
sl.streams.events.consume({ ...usdc.toStreamsParams(), onBatch });      // firehose
sl.subscriptions.create({ name, url, triggers: [usdc.toChainTrigger()] }); // webhook
...
sbtc.toIndexParams();    // type error: sBTC lifecycle events are Webhooks-only
```
...
// Subscriptions/Subgraphs-only; filter client-side on the decoded rows.
```
